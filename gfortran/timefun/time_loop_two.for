...
c      dimension syn(8192,21,3)
      ...
      double precision bvesave(516097)
...
c      enddo
      ...
      read(151,*) 
      read(151,*) 
      read(151,*) ifirst,istep,ilast ! myslim ze se pouziva jen formalne ??
      read(151,*) 
      read(151,*) isubmax
      read(151,*) 
      read(151,*) 
      read(151,*) f1,f2,f3,f4	 ! ano pozut v manidata  
      read(151,*)
      read(151,*) vardat
...
c	read(*,*) keymomcns
      ...
      if(nonetwo.eq.2) then
	write(*,*) 'requested SUB1 moment in Nm =?'     ! ??????????????????fixed moment???????????????????????????
 	read(*,*) xmomreq1								 ! ?????????????????????????????????????????????
	write(111,*) 'requested SUB1 moment in Nm = ', xmomreq1
 	write(*,*) 'requested SUB2 moment in Nm =?'     ! ??????????????????fixed moment???????????????????????????
 	read(*,*) xmomreq2								 ! ?????????????????????????????????????????????
	write(111,*) 'requested SUB2 moment in Nm = ', xmomreq2
 	else

	write(*,*) 'requested TOTAL moment in Nm =?'     ! ??????????????????fixed moment???????????????????????????
 	read(*,*) xmomreq12								 ! ?????????????????????????????????????????????
...
c      write(*,*) 'numdat= ',numdat
	...
	do j=1,numdat	  ! dulezite ULOZIT bve protoze se v tinv PREPISUJE !!
	bvesave(j)=bve(j)
	enddo
...
c	endif
	...
	isour1=0
	isour2=0
c	do 888 isour1=1,26
c	do 887 isour2=1,49
	do 888 isour1=1,ngridpul
...
c	enddo
	...
	do j=1,numdat	! dulezite vzit vzdy stare bve protoze se v tinv PREPISUJE !!
	bve(j)=bvesave(j)
	enddo

	call tinv(ama,bve,ampl,mode,rnorm,numdat,NTR2) ! neprepisuje se bve?
...
c      write(111,'(6(1x,e12.6))') (ampl(n),n=NTR+1,NTR2)
      ...
      call subCAS(ampl,w,sx,NTR2)   ! NEPROVERENO!!!!!!!!!!????????????????????
