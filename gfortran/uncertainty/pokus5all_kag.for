...
c	This code calculates the 6D error ellipsoid and its transformation into strike, dip, rake and VOL.

c      jako pokus3, ale perturbuji se TRI parametry (trebaze se zobrazuje 2D rez)

c   Co se plete? Volbou prvnich dvou parametru urcim rez elipsoidu ktey zobrazuji, napr a5 vs a6,
c   pokud me spec. zajima a6. Vektor delta_a tedy pro ZOBRAZENI ma jen 5 a 6. slozku.
c   Zajima me napriklad celkova neurcitost 6. parametru. Ta je dana sigma(a6)^2= suma (V6j/wj)^2 
c   kde se uplatnuji 6. slozky vsech vektoruu. Zde se uplatnuji jen 6. slozky vsech vektoru.
c   Zdalo by se tedy, ze ve skalarnim soucinu Vj. delta_a, ktery se vyhodnocuje pri vypoctu elipsoidu,
c   se uplatni jen delta a6 (ktere se nasobi vsemi V6j. Sesty parametr (a6) je ale svazan s dalsimi,
c   napr s 5. a 4. (konkretne s temi ktere odpovidaji velkym slozkam 6-teho vektoru). Proto je dulzeite
c   aby se pri vypoctu elipsoidu uplatnily vsechny vyznamne slozky 6. vektoru. Pokud je jich vic,
c   musim s nimi pocitat (napr 4. a 5. slozka) ale zobrazuji vzdy jen dve, napr 6 a 5, nebo 6 a 4.
c         
c   Priklad. Zajima me 6. vektor (protoze a6 souvisi s objemovou slozkou). Tento vektor ma 
c   nejvetsi slozy do 4., 5. a 6. osy. Sigma 6 je sice urcena prevqzne 6. slozkou 6 vektoru
c   ale to me nesplete ! Vim, ze musim vzit v uvahu i perturbace 4. parametru, ve skal soucinu
c   pri vypoctu elispoidu se uplatni 4 slzoka 6. vektoru jedine kdyz umoznim perturbace a4 !
c
c
c
...
c                                  !!!!!!!!!!! index2=6 a soucasne a(6)=ystep, tak l=-1 zpusobi y=a(6)=0 (vol=0 !)
      ...
      do k5=-5,5				   ! zde mozno casem dat volbu jaky rez se kresli
	do k6=-5,5				   ! soucasne nutno zmenit write(400 kde se to take voli !!!!
	x5=float(k5)*x5step
	x6=float(k6)*x6step
...
c        zde by se casem misto x5save a x6save kreslil ZVOLENY rez zatim 5 vodor 6 svisle     !!!!!!!
     ...
     * x5save(m),x6save(m),   zsave(m),avol_1,avol_2,
     *                                     abs(avol_2),
     *          str1,dip1,rake1,str2,dip2,rake2,rotangle       ! rotangle added here (column M)
...
c      changes for gfortran
c      SUBROUTINE SPHCOOR (QUAT, ANGL, THETA, PHI)
c    !returns rotation angle (ANGL) of a counterclockwise rotation
c    !and spherical coordinates (colatitude THETA and azimuth PHI) of the
c    !rotation pole. THETA=0 corresponds to vector pointing down.
c      IMPLICIT REAL*8 (A-H,O-Z)
c      REAL*8 QUAT(4)
c    
c      if(QUAT(4)<0.d0)QUAT(:)=-QUAT(:)
c      Q4N=dsqrt(1.d0-QUAT(4)**2)
c      COSTH=1.d0
c      if(dabs(Q4N)>1.d-10)COSTH=QUAT(3)/Q4N
c      if(dabs(COSTH)>1.d0)COSTH=IDINT(COSTH)
c      THETA=dacosd(COSTH)
c      ANGL=2.d0*dacosd(QUAT(4))
c      PHI=0.d0
c      if(dabs(QUAT(1))>1.d-10.or.dabs(QUAT(2))>1.d-10)
c     *               	 PHI=datan2d(QUAT(2),QUAT(1))
c      if(PHI<0.d0)PHI=PHI+360.d0
c    
c      END
     ...
     *        PHI=atan2(QUAT(2),QUAT(1))*rad2deg
