...
c      write(*,*) 'Non-normalized (0) or normalized, preferred (1) =?'
c      read(*,*) keynorm
       keynorm=1
c      write(*,*) 'normalization: ',keynorm
...
c      write(*,*) 'Incremental lag (as a multiple of dt)= ?'
c	  read (*,*) lagincr
      lagincr=10 
      nlag=ifix((2.*shiftplus/dt)/float(lagincr))
	  write(*,*) 'Number of tested lags= ',nlag
	  write(*,*) 'Incremental lag (in seconds): ',lagincr*dt
      endif
