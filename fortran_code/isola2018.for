...
c  ! final version of 2018 (dec. 31) 	  
c  ! ntim=8192 (fixed)
c  ! inv3c output file is added( = cummulative MT)   	  
c  ! Cd is not explicitly used (an constant-diagonal Cd is assumed, all diag. terms = vardat)	  
c  ! reads vardat from inpinv.dat	  
c  ! output of Cm into sing.dat, Cm = vardat * rinv, where  rinv=(GT G)-1
c  ! interaction disabled	  
c  ! can be used instead of 3 older codes: 'isola' and 'isola15unc' and also isola_jack
c  ! for interaction, use ISOLA2018_interaction
...
c     Author: J. Zahradnik 2003-2011-2015-2019
...
c     open(674,file='correl.dat')     ! 2d correlation (see later)
      ...
      open(900,file='inv3c.dat')     !       : cummulative moment tensor (for GMT) 

      open(300,file='sigma.dat')     
	  ! If 'open' is disabled here [but enabled later], the file will contain only the last subevent
	  ! If 'open' is  enabled here, the file will contain all subevents
	  ! which is unncessary in newunc for uncertainty (which takes each position separately
	  ! and for a given position all sigma's are same for all subevents)
      !	but may be useful in invert to see that different positions have different sigmas  
...
c      write(*,*) 'During pause you may check file CORRxx.DAT'
c      if(ipause.eq.1) PAUSE  ! not supported in gfortran
      if(ipause.eq.1) read *  ! good for gfortran 
...
c      open(300,file='sigma.dat')   
	  ! If 'open' is here, the file will contain only the last subevent
	  ! If 'open' is disabled here [but enabled at the beginning],the file will contain all subevents 
...
c	open(6789,file='vect.dat')	! vectors will be printed in columns
      ...
      do im=1,nmom
      write(300,'(6e15.6)') (vv(im,jm),jm=1,nmom) ! im...comp, jm... vector
      enddo
c	close(6789)

c	open(7789,file='sing.dat')	! sing values (inlcuding vardat), succession as the vectors
      write(300,*)
   	  write(300,*) 'sing. values'
      write(300,'(6e15.6)') (en(jm),jm=1,nmom) 
c 	close(7789)

										
c      Output of covariance matrix of parameters Cm = vardat * rinv
c      where  rinv=(GT G)-1

	  rinv=rinv*vardat		 

c      do i = 1,12; write(300,*); enddo ! 12 prazdnych radku aby bylo jako driv
      write(300,*)
	  write(300,*) ' Covariance matrix'
	  do i=1,nmom
      write(300,'(6e15.6)') (rinv(i,j)*1.e20,j=1,nmom) ! covariance corrected for scaling 1.e20
	  enddo		 

ccc	  do i=1,nmom; vopt(i)=sqrt(rinv(i,i)*1.e20); enddo !variances corrected for scaling
...
c      close(300) 										
	! If close is enabled here, the file will contain only the last subevent
...
c Newly added output of cummulative MT into  inv3c.dat
	 
      amoxx=-1.*aoptsum(4)+aoptsum(6)
      amoyy=-1.*aoptsum(5)+aoptsum(6)
      amozz=aoptsum(4)+aoptsum(5)+aoptsum(6)
      amoxy=aoptsum(1)
      amoxz=aoptsum(2)
      amoyz=-1.*aoptsum(3)
     ...
     *         abs(amort),abs(amorp),abs(amotp))
	  ...
	  write(900,89)      ! output of cummulative MT in inv3c.dat
     * iselect,ioptshf,
     * amorr,ia,amott,ia,amopp,ia,amort,ia,amorp,ia,amotp,ia
