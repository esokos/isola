...
c        This is combination of the former iso15_gen and sigma5or6_gen.
c        It needs waveform DATA (because makes non-lin unc estimate for depths).
c        It must be used only for a single depth of interest 
c        for which it calculates best-fitting MT and directly also cova matrix (file sigma.dat)
c        Vect and sing data are also in sigma.dat file. File testunc.dat is not generated.
c        Interactive regime is suppressed.
...
c     open(674,file='correl.dat')     ! 2d correlation (see later)
	  ...
	  ! If 'open' is  enabled here, [disabled later] it  will contain all subevents BUT THAT IS UNNECESSARY
      ! because sigma is used only in uncertainty, all sub at same position have same sigma 
...
c          Covariances of parameters (covariance matrix)
	...
	do i=1,nmom	 ! cykl pres slozky
	 ...
	 do j=1,nmom  ! cykl pres vektory
       parsum=(vv(i,j)*vv(k,j))/en(j)**2 	! v(i,j): i slozka, j vektor
c       parsum=(vv(i,j)*vv(k,j))/(en(j)*1.e20)**2 	! v(i,j): i slozka, j vektor
	 cov(i,k)=cov(i,k)+parsum ! you can check that SQUARED diagonal equals sigmas in inv1.dat
      enddo
	enddo
