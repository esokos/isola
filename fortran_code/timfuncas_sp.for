...
c    This version outputs complex spectrum spefc.dat 999! does not use auxshift
...
c
	...
	read(555,*) f1,f2,f3,f4
      ...
      write(*,*) 'artif.decrease of origin time (e.g. 10s)?' 
      read(*,*) auxshift
      auxshift=auxshift * (-1.)	  
...
c	auxshift=float(NT)*dt/2. !!! NEW Aug 2, 2015
...
c       U(I)=U(I)*cexp(-2*pi*ai*freq*(shift    ))   ! no time shift 
       U(I)=U(I)*cexp(-2*pi*ai*freq*(shift+auxshift)) !   
