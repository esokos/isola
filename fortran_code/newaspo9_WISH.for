      program NEWASPO9_WISH

c  WISH = Waveform Inversion With Shifts (without envelopes)

C Normalized waveforms are inverted for s/d/r by grid search.
c For each trial s/d/r, time shifts are applied to optimize the cross-correlation.
c The shifts are applied individually to every component.
c Moment is calculated for the best-fit s/d/r solution.
c The best-fit solution may NOT satisfy polarities (polarities need a posterior check).
c This code needs to be combined with sipolnew4_WISH for polarity check.
	  
c This is OLD text for newaspo9:	  
...
c      open(897,file='out0_inv1.dat')
      ...
      open(900,file='out3.dat')      ! opt GS solution (1 per all sources); no polarity check
...
c *******************************************************************
       ...
       call maniobs_bezobal(x,y1,y2,rr)    
...
c      sx is  synth  filtered displacement(incl. focmech)
      ...
      call prepsyn_bezobal(sx,y1,y2) ! envelope and ampl.sp. of the synth displacement filtered	
...
c      include "maniobs.inc"
      include "maniobs_bezobal.inc"
      include "subevnt15.inc"
      include "subevnt15lag.inc"	  
      include "elesyn.inc"
c      include "prepsyn.inc"
      include "prepsyn_bezobal.inc"
      include "genacka.inc"
