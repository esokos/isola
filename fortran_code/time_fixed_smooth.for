      program time_fixed2019
	  
c April 2019 adding smoothing constraint
	  
c 2019 stale pro powerstation a stale 8192; 
c prechod na verzi kompatibilni s greenovkami IFORT (napr pro 1024)
c by potreboval zmenit open(100...stream ) v elecas
c dale zmeny v timfuncas, a timfuncas1,2
c jinak samotna zmena 8192 na 1024 je snadna, neni nuno menit DIM, jen ntim=1024
...
c      parameter (nhdat=21*3*8192+1) ! max length of data column (1 = for moment); formerly 516097
      parameter (nhdat=21*3*8192+1+nhsrc) ! max length of data column (1 = for moment, nhsrc = for smoothing)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     *                 ff1(21),ff2(21),ff3(21),ff4(21),dt
      ...
      write(*,*) 'This is time_fixed_17 (new NNLS)'	
...
!     FIXED OPTIONS
	  ...
	  keysmo = 0
	  write(*,*) 'Do you want smoothing? 1/0 (=Y/N)'
	  read(*,*) keysmo
	  write(*,*) 'keysmo =',keysmo
...
c These 'limits' may vary from problem to problem. 
      ...
      bve(j)= bvecnst
	   ...
	   numdat=j
       write(*,*) 'data column: numdat before smooth= ',numdat

c -------------------- new April 2019 (smoothing) PART1, see also PART2
	  if(keysmo.eq.1) then
cccc first finite difference
c	  do irow=2,NTR2 
c      j=j+1			  
c      bve(j)= 0.
c      enddo	   

cccc second finite difference
	   do irow=2,NTR2-1 
       j=j+1			  
       bve(j)= 0.
       enddo	   
      ...
      numdat=j
      write(*,*) 'data column: numdat after smooth= ',numdat
...
!c     forming matrix: numdat rows, NTR2 columns
...
c Constraining moment (see description where bve is defined above)
...
c !!CAUTION the constant here (after (1./ xmomreq12) ) must be the same as in last data column bve 
     	...
     	numdat=j
c        write(*,*) 'matrix: numdat before smooth= ',numdat
    	
		enddo ! over it

c -------------------- new April 2019 (smoothing), PART2, see also PART1 (if editing...then BOTH!!)
      if(keysmo.eq.1) then 	
cccc first finite difference      POZOR problem by mohl byt kdyby fce 1 sla az do konce casoveho okna a fce2 zacinala na zacatku
c      do irow=2,NTR2	 ! adding NTR2-1 rows                     ! protoze hladim spojite za sebou obe casove rady jako jedinou 
c	  j=j+1			 
c        do jaux=1,NTR2
c        ama(j,jaux)=0.
c        enddo                  ! MULTIPLICATION by amamax is ESSENTIAL !!!!!!!!! if not, smoothing may overweight inversion
c	  ama(j,irow  )=+1 *amamax! * weig(ir,icom) ! *1.e10	 
c	  ama(j,irow-1)=-1 *amamax! * weig(ir,icom) ! *1.e10	 
c	  enddo ! over it = ober rows  
	  	  
cccc second finite difference   (there is no special preference of the 1st or 2nd difference) 
      do irow=2,NTR2-1	 ! adding NTR2-2 rows  
	  j=j+1			 
        do jaux=1,NTR2
        ama(j,jaux)=0.
        enddo
	  ama(j,irow+1)=+1 *amamax! * weig(ir,icom) ! *1.e10	 
	  ama(j,irow  )=-2 *amamax! * weig(ir,icom) ! *1.e10	 
	  ama(j,irow-1)=+1 *amamax! * weig(ir,icom) ! *1.e10	 
	  enddo ! over it = ober rows  
	  	  

      endif
      numdat=j 	  
c      write(*,*) 'matrix: numdat after smooth= ',numdat
