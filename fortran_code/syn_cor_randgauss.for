...
c check for 'fixed' options below !!!!!!!!!!!!! (BandPass, shift, baseline, trend)
      ...
      dimension bb(3,256),xpozice(256)
	  DIMENSION XAUX(256),TREND(256)
      dimension amax(3)
...
c
      ...
      xsecon=float(ntim)*dt
c      write(*,*) 'how many seconds of noise?' ! later add among data prepared by GUI
c      read(*,*) xsecon
      endif	
...
c     For RNNOR
	  ...
	  xvari=sqrt(xvari)
      if(icom.eq.3) write(*,*) 'noise sigma1', xvari
