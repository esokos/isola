      program NEWASPO9_dur

c As newaspo7, but with OPTIONAL NORMALIZATION (a new control param in INP.DAT)
c As newaspo8, but the non-used components are zeroed to prevent them as =1 after norm.
c As newaspo9, but controlling duration of inverted real waveforms
...
c for the best source position and s/d/r. This should be calcualated normally in ISOLA.
 
c Author: Jiri Zahradnik, Feb-Mar 2016, Prague
...
c      Depths (needed for inv1.dat plot) are taken from the GUI!
     ...
     *                 ff1(21),ff2(21),ff3(21),ff4(21),dt
      ...
      open(153,file='dur.dat')    ! input: control parameters

      open(894,file='ilags.dat')
      open(895,file='inv1_base.dat')
...
c**************************************************************
  ...
  7   read(150,*,end=8) statname(ir),nuse(ir),	 		
     ...
     *            ff1(ir),ff2(ir),ff3(ir),ff4(ir)   
      read(153,*,end=8) statname(ir),dur(ir) ! provizorni 
      stat(ir)=.true.
...
c *************************************************************************
      ...
      write(*,*) 'Non-normalized (0) or normalized (1) =?'
      read(*,*) keynorm
      write(*,*) 'normalization: ',keynorm
	  
c      write(*,*) 'Your Mw estimate =?'
c      read(*,*) xmw
c      estmom=10.**((xmw+6.03)*(3./2.))
c      write(*,*)'Estimate of Mw and Mo(Nm): ',xmw,estmom	  
       estmom=1.e15  ! FIXED OPTION edit (for Juraci, Mw 4) 
...
c      read(151,*) dpfrom,dpstep! trial depths (from, step; both in km)
      ...
      do ir=1,nr
	  ndur(ir)=ifix(dur(ir)/dt) !number of used points of real data
      enddo
...
c *******************************************************************
       ...
       call maniobs_dur(ndur,x,y1,y2,rr)    
...
c Power of the data rr 
      ...
      do ir=1,nr
c     do i=1,nump      ! time           ! NEW
      do i=1,ndur(ir)      ! time        ! NEW perhaps not needed when cutting in maniobs_dur
...
c and then test smaller shifts to find the best one. 
      ...
      read(*,*)  shiftOT	  
      write(*,*) 'Uncertainty (positive, in seconds)?'
      read(*,*)  shiftPLUS	  
...
c      xmo=estmom !	not as good as teh above; why ?  
      do ir=1,nr
      do i=1,nump
...
c      corrmax=corropt(ncount)	  
      write(894,*) isour          ! output of integer lags into a file
      do ir=1,nr
      ...
      write(894,'(2x,i5,5x,3(2x,i10))') ir,ix1,iy1,iz1
      enddo
      endif
...
c                                     ! to uz neni puvodni isour a indexopt 
      ...
      enddo
      write(894,*)      
      write(894,*) 'ilag (multiples of dt) for the best depth'
      write(894,*) isour          ! output of integer lags into a file
      do ir=1,nr
      write(894,'(2x,i5,5x,3(2x,i10))') ir,ilagx(ir),ilagy(ir),ilagz(ir)
      enddo      
      endif
...
c      include "maniobs.inc"
      include "maniobs_dur.inc"
