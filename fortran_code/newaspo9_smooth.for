      program NEWASPO10_smooth

c Dec 2017; smoothing of REAL amplitude spectra or envelopes  
c Probably NOT enough smooth for ENV, but OK for spectra ????!
...
c      character *100 text(27)
      ...
      dimension smx(8192),smy(8192),smz(8192)
...
c      Depths (needed for inv1.dat plot) are taken from the GUI!
     ...
     *                 ff1(21),ff2(21),ff3(21),ff4(21),dt
      ...
      write(*,*)
	  write(*,*) 'This is NEWaspo10_smooth'
	  write(*,*)
...
c      if(keyinv.eq.0) goto 2332
c     goto 2332	  
c****************************************************
c Observed DATA (spectra or envelopes) are smoothed

      do ir=1,nr
c
c     do i=3,nump-1     ! 3-point running average
c     smx(i)=(aox(i-1,ir)+aox(i,ir)+aox(i+1,ir))/3.
c     smy(i)=(aoy(i-1,ir)+aoy(i,ir)+aoy(i+1,ir))/3.
c     smz(i)=(aoz(i-1,ir)+aoz(i,ir)+aoz(i+1,ir))/3.
c     enddo
c     do i=3,nump-1
c     aox(i,ir)=smx(i)
c     aoy(i,ir)=smy(i)
...
c     enddo
c
      do i=5,nump-3       ! 7-point running average
      smx(i)=(aox(i-3,ir)+aox(i-2,ir)+aox(i-1,ir)+aox(i,ir)+
     *                    aox(i+1,ir)+aox(i+2,ir)+aox(i+3,ir))/7.
      smy(i)=(aoy(i-3,ir)+aoy(i-2,ir)+aoy(i-1,ir)+aoy(i,ir)+
     *                    aoy(i+1,ir)+aoy(i+2,ir)+aoy(i+3,ir))/7.
      smz(i)=(aoz(i-3,ir)+aoz(i-2,ir)+aoz(i-1,ir)+aoz(i,ir)+
     *                    aoz(i+1,ir)+aoz(i+2,ir)+aoz(i+3,ir))/7.
      enddo
      do i=5,nump-3
      aox(i,ir)=smx(i)
      aoy(i,ir)=smy(i)
