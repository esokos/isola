...
disp('plotjackrescomp_all 11/12/2022')
...
% open allstat new version only 
           [S,d1,d2,d3,d4,~,~,~,~] = textread('allstat.dat','%s %f %f %f %f %f %f %f %f',-1);
%   
...
for i=1:nstations
   ...
   if d1(i)~=0  % station is NOT disabled
        ...
        % add components
               switch j
                   case 1
                      alstname=['inv2_no_' S{i} '_NS.dat'];
                        ...
                        %  extract from inv1 VOL info 
                          alstname_inv1=['inv1_no_' S{i} '_NS.dat'];
                   ...
                   case 2
                      alstname=['inv2_no_' S{i} '_EW.dat'];
                        if d3(i)~=0
                        %  extract from inv1 VOL info 
                          alstname_inv1=['inv1_no_' S{i} '_EW.dat'];
                           fid3 = fopen(alstname_inv1,'r');
                             [~,~,~,~,~,~,inv1_vol,~,inv1_clvd,~,~,~,~]=readinv1new(alstname_inv1,nsources,selsub);
                           fclose(fid3); 
                   ...
                   case 3
                      alstname=['inv2_no_' S{i} '_Z.dat'];
                        ...
                        if d4(i)~=0
                        %  extract from inv1 VOL info 
                          alstname_inv1=['inv1_no_' S{i} '_Z.dat'];
                           fid3 = fopen(alstname_inv1,'r');
                             [~,~,~,~,~,~,inv1_vol,~,inv1_clvd,~,~,~,~]=readinv1new(alstname_inv1,nsources,selsub);
                           fclose(fid3); 
               ...
               end % switch j i.e. component

        end % loop of 3 components
...
subplot(2,5,7)
 hist(CLVD,linspace(0,100))
 xlabel('CLVD%')
 ylabel('Count')
v=axis;axis([0 100 v(3) v(4)])

subplot(2,5,8)
 hist(VOL,linspace(0,100))
 xlabel('ISO%')
 ylabel('Count')
v=axis;axis([0 100 v(3) v(4)])
