...
% Last Modified by GUIDE v2.5 22-Aug-2024 15:54:30
...
elseif str2num(linetmp2)==2
    ...
    set(handles.totalplot,'Enable','off')
else
    set(handles.full,'Value',0)
    set(handles.dev ,'Value',1) 
    set(handles.totalplot,'Enable','off')
...
% disable userdatavariance box
%set(handles.userdatavariance,'Enable','off');  
...
%% new copy hinv.dat file for uncertainty estimation
% if ispc 
%         fh2=exist('.\invert\hinv.dat','file'); 
%          if (fh2~=2) 
%                   errordlg('Invert folder doesn''t contain hinv.dat. Please run inversion. ','File Error');
%                   delete(handles.create_synth)
%          else
%              
%             disp('Copying invert\hinv.dat to newunc folder.')  
%             [status,message,~]=copyfile('.\invert\hinv.dat','.\newunc\hinv.dat');
%              
%          end
% else
%         fh2=exist('./invert/hinv.dat','file'); 
...
%              
%             disp('Copying invert\hinv.dat to newunc folder.')  
%             [status,message,~]=copyfile('./invert\hinv.dat','./newunc/hinv.dat');
%             
%          end
% end
...
elseif cova==1
 ...
 fclose(fid);
     ...
     disp('Copying invert\hinv.bin to newunc folder.')  
    [status,message,~]=copyfile('.\invert\hinv.bin','.\newunc\hinv.bin');
...
%%  proceed with loop
 cd .\newunc
    ...
    % remove done.tmp file before running the code
...
% prompt = "Scale factor? Try 1.0 [or 0.1 or 10]? Larger factor => larger spread";
% scalefact = input(prompt)

%Thimios modification
% prompt = {'Scale factor? Try 1.0 [or 0.1 or 10]? Larger factor => larger spread'};
% dlgtitle = 'Input Scale factor';
% dims = [1 35];
% definput = {'1'};
% answer = inputdlg(prompt,dlgtitle,dims,definput);
% scalefact=str2num(answer{1})
 scalefact=1;
...
norm_mo=max(mo);
disp(['Max Mo over all sources is ' num2str(max(mo),'%10.5e')]);
n_exp=floor(log10(norm_mo));


%% find maximum misfit misfitmin 

mis=zeros(nsources,1);

for i=1:nsources
    %inv1file =['inv4_src_'  num2str(i,'%02u') '.dat']; 
    %[~,~,~,~,inv1_mom,~,~,~,~,~,~,~,~]=readinv1new(inv1file,1,1); 
    inv4file =['inv4_src_'  num2str(i,'%02u') '.dat'];
    [sum_of_res2,~,~,~,~,~,~,misfit] = read_inv4_cova(inv4file);
    mis(i)=misfit;
end
misfitmin=min(mis)

%disp(['Max misfit over all sources is ' num2str(misfitmin,'%10.5e')]);
...
for i=1:nsources
     ...
     % inv1file =['inv1_src_'  num2str(i,'%02u') '.dat']; 
      ...
      %      [sum_of_res2,~,~,~,~,~,~] = read_inv4(inv4file); % OLD
      % calculate misfit                                      % OLD
      % misfit=sum_of_res2/var;                               % OLD
             [sum_of_res2,~,~,~,~,~,~,misfit] = read_inv4_cova(inv4file); % NEW    JZ
%             if i==1
%             mconst=misfit;
%             end
%            misfit=misfit/mconst;
            misfit=misfit-misfitmin;
      % read from files the parameters we need
      C = read_sigma2(sigmafile,stype);
      ...
      %if ncova == 1
         ...
         %disp(['NOT Divide COVA matrix by ' num2str(((10^n_exp)^2),'%5.2e')]);
      %else
      %end
...
for i=1:nsources
    ...
    % inv1file =['inv1_src_'  num2str(i,'%02u') '.dat']; 
      ...
      % calculate misfit                                      % OLD
      % misfit=sum_of_res2/var; % OLD
              [sum_of_res2,~,~,~,~,~,~,misfit] = read_inv4_cova (inv4file); % NEW    
%             if i==1
%             mconst=misfit;
%             end
%            misfit=misfit/mconst;      
    ...
    C = read_sigma2(sigmafile,stype);
      ...
      if stype==1 % full MT
         ...
         %  get str dip rake etc    
          [m,~]=size(r);
          % code without signed CLVD
          %             for ii=1:m
          %                 [str1(ii),dp1(ii),rk1(ii),str2(ii),dp2(ii),rk2(ii),adc_1(ii),adc_2(ii),avol_1(ii),avol_2(ii)] = silsubnew(r(ii,:));
          %             end
          % code with  signed CLVD
            for ii=1:m
                [str1(ii),dp1(ii),rk1(ii),str2(ii),dp2(ii),rk2(ii),adc_1(ii),adc_2(ii),avol_1(ii),avol_2(ii),aclvd_1(ii),aclvd_2(ii),amt(ii)] = silsubnew2(r(ii,:));
            end
             
            total_r=cat(1,total_r,r);
            total_req6=cat(1,total_req6,req6);
              
            clear r req6 str1  dp1  rk1  str2  dp2  rk2  adc_1  adc_2  avol_1  avol_2  amt AA BB n xout nv vxout path 
         ...
         % [a1,a2,a3,a4,a5,a6]=sdr2as(strike1,dip1,rake1,xmoment*1e-20);  % scaling of moment..! scaling is used by Jiri in fortran
          ...
          r = mvnrnd(A,sigma,fix_samples);  %%% be careful here we use fixed=200 no of samples
          r=[r zeros(fix_samples,1)];        %%% be careful here we use fixed=200 no of samples
          
          req6= mvnrnd(A,sigma,new_no_of_rand_samples);  %%% get r using newNi
          req6=[req6 zeros(new_no_of_rand_samples,1)];   %%% get r using newNi
          
          %%  get str dip rake etc  
            [m,~]=size(r);
            for ii=1:m
                [str1(ii),dp1(ii),rk1(ii),str2(ii),dp2(ii),rk2(ii),adc_1(ii),adc_2(ii),avol_1(ii),avol_2(ii),amt(ii)] = silsubnew2(r(ii,:));  % new code for signed CLVD
            end  

             total_r=cat(1,total_r,r);
             total_req6=cat(1,total_req6,req6);
             clear r str1  dp1  rk1  str2  dp2  rk2  adc_1  adc_2  avol_1  avol_2 amt AA BB n xout path
...
%% plot total 
            ...
            % 
            %  for ii=1:m
            %      [str1(ii),dp1(ii),rk1(ii),str2(ii),dp2(ii),rk2(ii),adc_1(ii),adc_2(ii),avol_1(ii),avol_2(ii),aclvd_1(ii),aclvd_2(ii),amt(ii)] = silsubnew2(total_r(ii,:));
            %  end  
            % 
            % clear str1 dp1 rk1 str2 dp2 rk2 adc_1 adc_2 avol_1 avol_2  aclvd_1  aclvd_2 amt
%%
%     eq6            
              [meq6,~]=size(total_req6);
             
              disp(['Total samples (based on eq6) = ' num2str(meq6)])
              str1=zeros(1,meq6);dp1=zeros(1,meq6);rk1=zeros(1,meq6);str2=zeros(1,meq6);dp2=zeros(1,meq6);rk2=zeros(1,meq6);
              adc_1=zeros(1,meq6);adc_2=zeros(1,meq6);avol_1=zeros(1,meq6);avol_2=zeros(1,meq6);aclvd_1=zeros(1,meq6);aclvd_2=zeros(1,meq6);amt=zeros(1,meq6);
...
%             
             if stype==1 % full MT
                figure; %DC_1
                subplot(1,5,1)
                histogram(adc_2,dcbins);title('DC% all sources  ','FontSize',14);
                ax=gca;set(ax,'Linewidth',2,'FontSize',12);xlim([0,100]);

                subplot(1,5,3)  %avol_1
                histogram(avol_2,xbins);title('VOL% all sources ','FontSize',14);
                ...
                hold off               
                %%
                P_y=zeros(1,meq6);P_x=zeros(1,meq6);T_y=zeros(1,meq6);T_x=zeros(1,meq6);
...
%               legend('P','T','Location','northwest')
                ...
                subplot(1,5,2)  %
                histogram(aclvd_2,xbins);title('CLVD% all sources','FontSize',14);
                ax=gca;set(ax,'Linewidth',2,'FontSize',12);xlim([-100,100]);
                ...
                Mxx=-a4+a6; Myy=-a5+a6; Mzz=a4+a5+a6;
                Mxx_Myy=Mxx./Myy;  Mxx_Mzz=Mxx./Mzz;
                %  whos
                % add a header
                fid2=fopen('CLVD_ISO_DC_Mo_MW_eq6.dat','w');
...
%% enable source type plot IF we have FULL MT ..!!

if stype==1  %full
    set(handles.hudson,'Enable','On')
    set(handles.hudsonpersource,'Enable','On')
else
...
set(handles.samplespersource,'Enable','On')
...
% sum of all sources red thick curve
% plot(centers,nofix,'Linewidth',2,'Color', [1 0 0])

%% plot of weighted sum using eq.6
     fid=fopen('vol_histout_all_eq6.dat');
       Cw6=textscan(fid,'%f %f');
     fclose(fid);
       plot(Cw6{2},Cw6{1},'b','Linewidth',1.5)
     clear Cw6   
...
for ii=1:nsources
     ...
     fid=fopen(histfilename);
      C=textscan(fid,'%f %f');
     fclose(fid);
%    plot of individual source red thin curve     
     plot(C{2},C{1},'r')
     clear C 

%  add plot from constant samples = 200 gray curve
...
legend('Weighted Sum','Weighted','Non-Weighted','Location','northwest')
...
% Hint: get(hObject,'Value') returns toggle state of full
    ...
    set(handles.dev ,'Value',0)
    set(handles.totalplot,'Enable','on')
...
% --- Executes on button press in samplespersource.
function samplespersource_Callback(hObject, eventdata, handles)
% hObject    handle to samplespersource (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% this will create a plot of number of samples per source
% we need to know how many source we have

nsources=str2num(handles.linetmp6);   
disp(['Running with ' num2str(nsources) ' trial source positions.'])

% check if log file exists
% we assume newunc folder exists

if exist('.\newunc\newunc_log.dat','file')

    % read the file 
    fid=fopen('.\newunc\newunc_log.dat');
     first_part=textscan(fid,'%u %f %f %f %f %f',nsources,'HeaderLines',1); 
     second_part=textscan(fid,'%u %u',nsources);
    fclose(fid);
...
sampl=cell2mat(second_part(2))

    bar(sampl,'r','FaceColor',[0 .9 .9],'EdgeColor',[0 .5 .5],'LineWidth',4)
    title('Number of samples versus trial positions','FontSize',14)
    xlabel('Trial source')
    ylabel('Number of samples')
    ax=gca;set(ax,'Linewidth',2,'FontSize',12) 
    
else
   errordlg('newunc_log.dat file not found','File Error');
end


% --- Executes on button press in hudsonpersource.
function hudsonpersource_Callback(hObject, eventdata, handles)
% hObject    handle to hudsonpersource (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

sourceplotuncnewpersource  % it will plot STP with color per source
