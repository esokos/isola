...
disp('This is plotres 02/12/2022');
disp('Changes:  use of freqchk.isl');
...
%% 
%% check if we have the freqchk.isl file

if isfile('freqchk.isl')
    disp('freqchk.isl FOUND ')
   % read the file
   fid = fopen('freqchk.isl','r');
    a=fscanf(fid,'%u'); 
   fclose(fid)

   if a==0 % different freq band per station
       invband='\bf \color{red} Station-dependent';
   else
   end
    
else
    disp('freqchk.isl missing ')
end
...
if gmt_ver==4
   ...
   %% put code for GMT5 ! we will update this option from now on .. 

   if nostations <=21
      eh7 = uimenu(mh,'Label','Prepare a GMT script','Callback','makegmtscript4synobsplot5'); 
   else
      eh7 = uimenu(mh,'Label','Prepare a GMT script','Callback','makegmtscript4synobsplot5_more21'); 
   end
...
%%%%%%%%%%%%%%%%%%%%%%%%  check if source is under Moho (the last interface is better term !!!!)...!!we need to add
%%%%%%%%%%%%%%%%%%%%%%%%  another layer then .........

disp(['Found last interface at ' num2str(model_depths(nlayers)) '  km'])
disp(['Source depth is ' num2str(depth) '  km'])

if depth >= model_depths(nlayers)

    disp('Source depth is deeper than the the last interface. Adding one layer in crustal.dat.')
