function [] = jackstations_cova

%[~,~,~,~,cova] = readisolacfg;
% keep cova type fixed for now
...
cd invert
   ...
   if exist('done', 'file')==2
        delete('done');
   end
   
     
%%     
     % create two batch files first ..!! one with and one without cova
     if ispc
         
         fid = fopen('runisolajacknocova.bat','w');
         ...
         fid = fopen('runisolajackcova.bat','w');
               fprintf(fid,'%s\r\n','call "C:\Program Files (x86)\Intel\oneAPI\setvars.bat"');
               fprintf(fid,'%s\r\n','isola_cova.exe');
               fprintf(fid,'%s\r\n','norm.exe');
               fprintf(fid,'%s\r\n','done.exe');
...
%%    first run should be without COVA 
     if ispc   
         system('runisolajacknocova.bat')    
     else
         %         
     end
      %  wait here for fortran part to end
         ...
         disp('         ')
         disp('Fortran no_cova code finished.')
         disp('         ')
         delete('done'); 
     
%%   now run the matlab code to create hinv.bin
           if strcmp(buttoncova,'COMPVAR')
                  disp('Running prep_COMPVAR')
                  prep_COMPVAR 
           elseif strcmp(buttoncova,'FULL')
                  disp('Running prep_FULL')
                  prep_FULL_jack 
           elseif strcmp(buttoncova,'AXCF')
                  prep_AXCF
           elseif strcmp(buttoncova,'Cancel')
             disp('Canceled ')
             cd ..
             pwd
             return
...
%%    RUN THE COVA FORTRAN CODE
     if ispc   
         system('runisolajackcova.bat')   
     else
     ...
     %  wait here for fortran part to end
       ...
       disp('         ')
       disp('Fortran cova code finished.')
       disp('         ')
       delete('done'); 
       
%% now take care of output..!! copy needed files in jack folder
    if ispc    
