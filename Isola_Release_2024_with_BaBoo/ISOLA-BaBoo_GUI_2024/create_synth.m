...
%% Nyquist
nyqst=1/(2*tstep);
set(handles.nyquist,'String',num2str(nyqst,'%2.2f'))  
...
realdataall=zeros(npts,4,nostations);    %%%% 1024 points  fixed.... use npts now based on defaults!
...
%we expect data in form 1024x4 1st column time 2nd NS etc...
...
if length(index_selected)==1   %%% plot ONE or ALL stations ONLY
    ...
    if index_selected == nostations+1                          %%%ALL
          ...
          plotallstationssyn(nostations,staname,uselimits,ftime,totime,keyheader,npts)
    elseif index_selected ~= nostations+1                       %%% just one
          disp('Plotting ONLY One Station Synthetic data in one Figure')
          stationname=staname{index_selected}
          plotonestationsyn(stationname,uselimits,ftime,totime,keyheader,npts)
    end
...
function plotallstationssyn(nostations,staname,uselimits,ftime,totime,keyheader,npts)
%%%%%prepare filenames  
%%% we keep fixed that data will be e.g. evrfil.dat
%%% and synthetics will be               evrsyn.dat
    ...
    %  
        ...
        if length(nsunevt)~=1
           warndlg('Found more than one subevents, the first will be used.','!! Warning !!')
            set(handles.Mrr,'String',num2str(C{1,3}(1),'%9.4e'));
            set(handles.Mtt,'String',num2str(C{1,4}(1),'%9.4e'));
            set(handles.Mpp,'String',num2str(C{1,5}(1),'%9.4e'));
            set(handles.Mrt,'String',num2str(C{1,6}(1),'%9.4e'));
            set(handles.Mrp,'String',num2str(C{1,7}(1),'%9.4e'));
            set(handles.Mtp,'String',num2str(C{1,8}(1),'%9.4e'));
        else            
            set(handles.Mrr,'String',num2str(C{3},'%9.4e'));
            set(handles.Mtt,'String',num2str(C{4},'%9.4e'));
            set(handles.Mpp,'String',num2str(C{5},'%9.4e'));
            set(handles.Mrt,'String',num2str(C{6},'%9.4e'));
            set(handles.Mrp,'String',num2str(C{7},'%9.4e'));
            set(handles.Mtp,'String',num2str(C{8},'%9.4e'));
        end
