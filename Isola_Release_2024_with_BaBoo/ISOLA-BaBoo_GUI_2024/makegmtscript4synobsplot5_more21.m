function result = makegmtscript4synobsplot5_more21

disp('This is makegmtscript4synobsplot5_more21 30/04/2024')
...
%% Open batch file
 fid = fopen('plobs_syn.bat','w');
         ...
         fprintf(fid,'%s\r\n','gmtset FONT_ANNOT_PRIMARY 8 FONT_ANNOT_SECONDARY 8  FONT_TITLE 8 FONT_LABEL 8  PS_MEDIA A3 '); %%% D_FORMAT %%4.1e
...
%% loop over stations
 for i=1:nostations-1
       ...
       if i==1            %% first line 
           ...
           if compuse(1,i)==1 
           fprintf(fid,'%s \r\n',['gawk "{print $1,$2}" ' realdatafilename{i} ' | psxy  -R' num2str(ftime) '/' num2str(totime) '/' num2str(maxmindataindex(1,2,i)) '/' num2str(maxmindataindex(1,1,i)) '  -JX5c/1.c -K %obspen% -P -B100/' num2str((abs(maxmindataindex(1,2,i))+maxmindataindex(1,1,i))/3) '::wesn -V -Y38c > %psfile% ']);
