...
disp('This is mseedimport 30/04/2024');
disp('Based on a) RDMSEED code of François Beauducel and b) ReadMSEEDFast code by Martin Mityska');
disp('Both available from Matlab file exchange.');
...
nssamples=handles.nssamples;
%% 

cd(path1)

[file1,path1] = uigetfile('*.*','Import mseed file');
...
if chan2(3)~='E'
    waitfor(warndlg('This doesn''t look like EW component file. This is just a warning.If you are sure about component orientation you can go on','!! Warning !!'))
...
if nssamples~=maxnssamples
    set(handles.ewaxis,'Color','yellow')
    disp('NS and EW don''t have the same number of points. Use cut')
else
end
...
%%%%% isola path
path1=handles.path1; mainpath=handles.mainpath;
...
%%  Decide if this is a multiplexed file
% check size of II
[multi,~]=size(signal);


if multi==3  % 3 channel multiplexed mseed file

  disp('Found 3 channels in file')
  channels= extractfield(signal,'channel');
  

  for jj=1:3  % loop through channels

      % check which component is this....  extract the last letter 
      currentchanel=channels{jj};  compname= currentchanel(end);

     switch compname
         
         case 'E'
           disp(['Found  ' compname  ' component'])
           % get data for EW
           samples_ew=signal(jj).data;   dt = 1/signal(jj).sampleRate;  sta=signal(jj).station;chan2=signal(jj).channel; 
           set(handles.ewname,'String',[sta '  ' chan2]);  set(handles.sfreq,'String',num2str(1/dt))

           % find start time
           starttime= datetime(signal(jj).matlabTimeVector(1),"ConvertFrom","datenum");
           set(handles.ewtime,'String',[char(starttime,'dd-MMM-yyyy') ' (' num2str(day(starttime,'dayofyear'),'%03u') ') '  char(starttime,'HH:mm:ss.SSS')]);
           
           %this is the sample number ....
           maxsamples=signal(jj).numberOfSamples; tim=(1:1:maxsamples); time_sec=((tim.*dt)-dt)';
           % plot
           axes(handles.ewaxis);   plot(time_sec,samples_ew,'k') ;    set(handles.ewaxis,'XMinorTick','on');grid on;xlabel('Time (sec)');ylabel('Counts')

           starttimeEW=starttime;

         case 'N'
            disp(['Found  ' compname  ' component'])
           % get data for NS
           samples_ns=signal(jj).data;   dt = 1/signal(jj).sampleRate;  sta=signal(jj).station;chan2=signal(jj).channel; 
           set(handles.nsname,'String',[sta '  ' chan2])

           % find start time
           starttime= datetime(signal(jj).matlabTimeVector(1),"ConvertFrom","datenum");
           set(handles.nstime,'String',[char(starttime,'dd-MMM-yyyy') ' (' num2str(day(starttime,'dayofyear'),'%03u') ') '  char(starttime,'HH:mm:ss.SSS')]);
           
           %this is the sample number ....
           maxsamples=signal(jj).numberOfSamples; tim=(1:1:maxsamples); time_sec=((tim.*dt)-dt)';
           % plot
           axes(handles.nsaxis);   plot(time_sec,samples_ns,'k') ;    set(handles.nsaxis,'XMinorTick','on');grid on;xlabel('Time (sec)');ylabel('Counts')
         ...
         case 'Z'
           ...
           % get data for Z
           samples_ver=signal(jj).data;   dt = 1/signal(jj).sampleRate;  sta=signal(jj).station;chan2=signal(jj).channel; 
           set(handles.vername,'String',[sta '  ' chan2])

           % find start time
           starttime= datetime(signal(jj).matlabTimeVector(1),"ConvertFrom","datenum");
           set(handles.vertime,'String',[char(starttime,'dd-MMM-yyyy') ' (' num2str(day(starttime,'dayofyear'),'%03u') ') '  char(starttime,'HH:mm:ss.SSS')]);
           
           %this is the sample number ....
           maxsamples=signal(jj).numberOfSamples; tim=(1:1:maxsamples); time_sec=((tim.*dt)-dt)';
           % plot
           axes(handles.veraxis);   plot(time_sec,samples_ver,'k') ;    set(handles.veraxis,'XMinorTick','on');grid on;xlabel('Time (sec)');ylabel('Counts')

           starttimeZ=starttime;

     end 

  end

   % we should enable/disable buttons and update handles here
   % save RAW data to handles
   ...
   % save all 3 channels
   handles.ns = samples_ns;    handles.ew = samples_ew;    handles.ver = samples_ver;    handles.dt=dt;

   % this is for compatibility with old code 
   dvNS=datevec(starttimeNS);    istns=datenum(dvNS);    handles.istns=istns;    handles.timerefns=istns;
   dvEW=datevec(starttimeEW);    istew=datenum(dvEW);    handles.istew=istew;    handles.timerefew=istew;
    dvZ=datevec(starttimeZ) ;     istZ=datenum(dvZ) ;    handles.istver=istZ;    handles.timerefver=istZ;
   ...
   % handles off we disable allign we expect in this case to have same
   % start time
   set(handles.Allign,'Enable','off')

   %% check if data start time is the same for all channels

    if starttimeNS ~=starttimeEW
          disp('NS~EW')
          set(handles.ewtime,'ForegroundColor','red')
          set(handles.nstime,'ForegroundColor','red')
          set(handles.Allign,'Enable','on')
    end
    if starttimeNS~=starttimeZ
          disp('NS~Z')
          set(handles.nstime,'ForegroundColor','red')
          set(handles.vertime,'ForegroundColor','red')
          set(handles.Allign,'Enable','on')
    end
    if starttimeEW~=starttimeZ
          disp('EW~Z')
          set(handles.ewtime,'ForegroundColor','red')
          set(handles.vertime,'ForegroundColor','red')
          set(handles.Allign,'Enable','on')
...
%%  single channel mseed 
elseif multi==1 % single component

    samples_ns=signal.data;   dt = 1/signal.sampleRate;  sta=signal.station;   chan2=signal.channel; 
    set(handles.sfreq,'String',num2str(signal.sampleRate))

    % find start time
    starttime= datetime(signal.matlabTimeVector(1),"ConvertFrom","datenum");
    set(handles.nstime,'String',[char(starttime,'dd-MMM-yyyy') ' (' num2str(day(starttime,'dayofyear'),'%03u') ') '  char(starttime,'HH:mm:ss.SSS')]);
           
    %this is the sample number ....
    maxsamples=signal.numberOfSamples; tim=(1:1:maxsamples); time_sec=((tim.*dt)-dt)';
    
   %
   if chan2(3)~='N'
       waitfor(warndlg('This doesn''t look like NS component file. This is just a warning. If you are sure about component orientation you can go on','!! Warning !!'))
       set(handles.nsname,'String',[sta '  ' chan2])
       set(handles.nsname,'ForegroundColor','r')
       set(handles.nsname,'BackgroundColor','y')
   else
       set(handles.nsname,'String',[sta '  ' chan2])
   end
   ...
   % this is for compatibility with old code 
   dv=datevec(starttime);    istns=datenum(dv);    handles.istns=istns;    handles.timerefns=istns;


   handles.istns=istns;
   handles.dt=dt;
   handles.timerefns=istns;
   handles.nssamples=maxsamples;
   guidata(hObject,handles)
...
elseif multi>3 % more than one station in multiplexed file 

   errordlg('Miniseed import tool cannot handle multiplexed files for many stations.','Error')
   disp(['It seems that this file has '  num2str(multi) ' channels.' ])
...
if chan2(3)~='Z'
    waitfor(warndlg('This doesn''t look like Z component file. This is just a warning.If you are sure about component orientation you can go on','!! Warning !!'))
    set(handles.vername,'String',[sta '  ' chan2])
...
if nssamples~=maxnssamples
    set(handles.veraxis,'Color','yellow')
    disp('NS and Z don''t have the same number of points. Use cut')
else
end
...
istns=handles.istns;  istew=handles.istew; istver=handles.istver;

ew=handles.ew ; ns=handles.ns ; ver=handles.ver; dt=handles.dt;
...
if Y(1,3) ~= Y(2,3) || Y(1,3) ~= Y(3,3)
    disp('Error in file DATES, difference is too big.')
end

%%  find maximum time and cut

[MAXTIME,b]=max([istns;istew;istver]);

[M(1),M(2),M(3),Y(4),Y(5),Y(6)] =datevec(MAXTIME); 
disp(['Maximum time is ' sprintf('Date: %d/%d/%d   Time: %d:%d:%2.3f\n', M(1),Y(2), Y(3),Y(4),Y(5),Y(6))])

%disp(['Maximum time is   ' ])

if b ==1
    %%allign to ns
    %%find cutsamples
    disp('Alling to NS time')
