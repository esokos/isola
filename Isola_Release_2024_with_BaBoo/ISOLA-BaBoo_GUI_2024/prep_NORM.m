...
%  end % END of IF plotting seismo
 ...
 % new plot
 % subplot1(i+pp)
 % plot(TDATplot) ; title(['standardized DAT blue, non-stand DAT red at station  ' statcodes{i}])
 % hold on
 % plot(DATplot,'r')
 % hold off

 subplot(numstat,5,i*5-3)
 plot(DATplot)
 %[t,~]=title(['NON standardized DAT  blue, NON-stand SYN red at station  ' statcodes{i}]) %;t.FontSize = 10;
 HT=title(['NON standardized DAT  blue, NON-stand SYN red at station  ' statcodes{i}]);HT.FontSize=10;


 hold on
 plot(SYNplot,'r')
 xline(a,'--k','LineWidth',2); xline(a*2,'--k','LineWidth',2);
 hold off

 subplot(numstat,5,i*5-2)
 plot(TDATplot)
 %[t,~]=title(['standardized DAT blue and standardized SYN red at station' statcodes{i}]);t.FontSize = 10; 
 HT=title(['standardized DAT blue and standardized SYN red at station' statcodes{i}]);HT.FontSize=10;
...
%  end % End of IF plot for station selstnum PLOTTING of SPECTRA
      ...
      subplot(numstat,5,i*5-1)
      semilogy(freq,spDATplot);xlabel('Hz')
      %[t,~]=title(['SPECTRA DAT non-stand blue, SYN non-stand red at station ' statcodes{i}]);t.FontSize = 10;
      HT=title(['SPECTRA DAT non-stand blue, SYN non-stand red at station ' statcodes{i}]);HT.FontSize=10;
      ...
      subplot(numstat,5,i*5)
      semilogy(freq,spTDATplot);xlabel('Hz'); 
      %[t,~]=title(['SPECTRA DAT stand blue, SYN stand red at station ' statcodes{i}]);t.FontSize = 10;
      HT=title(['SPECTRA DAT stand blue, SYN stand red at station ' statcodes{i}]);HT.FontSize=10;
...
% if(i==selstnum)
 ...
 subplot(numstat,5,i*5-4)
 plot(Taplot) 
 %[t,~]=title(['corrrow100, stand blue, non-stand red stat :  ' statcodes{i}]); t.FontSize = 10;
 HT=title(['corrrow100, stand blue, non-stand red stat :  ' statcodes{i}]);HT.FontSize=10;
