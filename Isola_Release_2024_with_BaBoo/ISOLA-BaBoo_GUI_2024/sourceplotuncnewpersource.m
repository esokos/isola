function ok = sourceplotuncnewpersource
%% Modification to plot symbols colored according to trial source position 
% 15/08/2024
...
%AXYa=zeros(length(alpha_depth{kk}),3);
for kk=1:nsources
 ...
 if ~isempty(alpha2)
     ...
     [MTsamples,~]=size(alpha_depth{kk});  % the number of rows with a's aka the number of MTs  ...
     disp(['Plotting ' num2str(MTsamples) ' MT samples for source no  ' num2str(kk)])

   for ii=1: MTsamples   %length(alpha_depth{kk})
     ...
     [~,L] = eigsort(MT);     M1 = L(1,1);     M2 = L(2,2);     M3 = L(3,3);
     %% plot
     AXYa(ii,:)=SourceType([M1 M2 M3],'Cubic',0,0);
%    scatter(AXYa(:,2),AXYa(:,3),2,CC(kk,:),'Marker','o')
     ...
     disp(['Source no  ' num2str(kk)  ' has 0 MT samples'])
 end
