...
% Last Modified by GUIDE v2.5 26-Nov-2020 14:40:37
...
%                 xlim([-100,100]);
                   [nv,vxout] = hist(avol_2,xbins);
                    ...
                    fclose(fid2);                    
%%
                   [nv,vxout] = hist(aclvd_2,xbins);
                   
                    histfilename=['clvd_histout_src' num2str(i,'%02u') '_CONST.dat'];
                    fid2 = fopen(histfilename,'w');
                        fprintf(fid2,'%f  %f \r\n',[nv; vxout]);
                    fclose(fid2);
                    
                    histfilename2=['clvd_histout_src' num2str(i,'%02u') '.dat'];
                    fid2 = fopen(histfilename2,'w');
                        fprintf(fid2,'%f  %f \r\n',[nv*(new_no_of_rand_samples/fix_samples); vxout]);
                    fclose(fid2);                      
...
%           end
       ...
       %%  get str dip rake etc  
            [m,~]=size(r);
            for ii=1:m
                [str1(ii),dp1(ii),rk1(ii),str2(ii),dp2(ii),rk2(ii),adc_1(ii),adc_2(ii),avol_1(ii),avol_2(ii),amt(ii)] = silsubnew(r(ii,:));
            end  
...
%                 %figure;
%                 subplot(1,2,2)
%                 Stereonet(0,90*pi/180,1000*pi/180,1);8
% %                 v=axis;
% %                 xlim([v(1),v(2)]); ylim([v(3),v(4)]);  % static limits
%                 axis ([-1 1 -1 1]);
%                 hold on
...
%                 hold off
                ...
                %% output CLVD
                fid2 = fopen('output_all_CLVD.dat','w');
                 fprintf(fid2,'%f\r\n',aclvd_2);
                fclose(fid2);
                
                [n,xout] = hist(aclvd_2,xbins);
                fid2 = fopen('clvd_histout_all.dat','w');
                 fprintf(fid2,'%f  %f \r\n',[n; xout]);
                fclose(fid2);
...
%                legend('P','T','Location','northwest')
                ...
                figure
                hist(aclvd_2,xbins);title('CLVD% all sources','FontSize',14);
                ax=gca;set(ax,'Linewidth',2,'FontSize',12);xlim([-100,100]);
                
                fid2 = fopen('output_all_eq6_clvd.dat','w');
                 fprintf(fid2,'%f\r\n',aclvd_2);
                fclose(fid2);
                [n,xout] = hist(aclvd_2,xbins);
                fid2 = fopen('clvd_histout_all_eq6.dat','w');
                 fprintf(fid2,'%f  %f \r\n',[n; xout]);
                fclose(fid2);                
