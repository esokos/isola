function ff = hudsonnet(add,addpoints,addtext)
...
ff=figure('Name','Source Type Plot','Color','w');
hold on
...
for i=1:length(Ts)
    ks0=(-1:0.01:1);
    u0 = zeros(length(ks0),1);
    v0 = zeros(length(ks0),1);
    for j=1:length(ks0)
        [u0(j),v0(j)]=tk2uv(Ts(i),ks0(j));
    end
    line(u0,v0,'LineStyle',':','Color', [0.3010 0.7450 0.9330])
end

for i=1:length(ks)
    Ts0=(-1:0.01:1);
    u0 = zeros(length(Ts0),1);
    v0 = zeros(length(Ts0),1);
    for j=1:length(Ts0)
        [u0(j),v0(j)]=tk2uv(Ts0(j),ks(i));
    end
    line(u0,v0,'LineStyle',':','Color', [0.3010 0.7450 0.9330])
end
...
line([hp1u hp2u],[hp1v hp2v],'Color', [0.3010 0.7450 0.9330]);
line([vp1u vp2u],[vp1v vp2v],'Color', [0.3010 0.7450 0.9330])
line([-4/3,4/3],[-1/3,1/3],'Color', [0.3010 0.7450 0.9330])


%% plot edge lines

line([p1(1),p2(1)],[p1(2),p2(2)],'Color', [0.3010 0.7450 0.9330]);
line([p2(1),p3(1)],[p2(2),p3(2)],'Color', [0.3010 0.7450 0.9330]);
line([p3(1),p4(1)],[p3(2),p4(2)],'Color', [0.3010 0.7450 0.9330]);
line([p4(1),p1(1)],[p4(2),p1(2)],'Color', [0.3010 0.7450 0.9330]);
