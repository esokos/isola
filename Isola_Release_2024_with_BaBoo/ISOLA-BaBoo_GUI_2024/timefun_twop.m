...
%    mom1=fscanf(fid,'%s',1);
 ...
 %   set(handles.mo2,'String',mom2);
    ...
    set(handles.totalmo,'String',totalmo);
    ...
    % put dt as default in trdist ! 29/11/2022
     trdist=linetmp4;
     set(handles.trdist,'String',trdist);
end
%%
...
totalMo=get(handles.totalmo,'String');
...
%% end of source configuration line or plane..!!  
    ...
    warndlg('Ploting not available for this trial source configuration. Try line or plane source.','Warning');
...
if get(handles.samesources,'Value') == 1
   ...
   % prepare the two acka files
     ...
     %  use same str dip rake for source 2 also 
...
if(get(handles.cMo,'Value')==1)
    ...
    % check if it is empty and issue warning 28/11/2022
    if isnan(moment)
     f = errordlg('Please specify Total Moment value','Error');
     waitfor(f);
    else
...
srcA=str2num(char(answer(1)));srcB=str2num(char(answer(2)));

%% we should prepare tfuninp.dat now
% read duration 
tdur=str2num(get(handles.tdur,'String'));
starttime=str2num(get(handles.tshift,'String'));
trdist=str2num(get(handles.trdist,'String'));
...
%system('tfun.exe ');
%system('done.exe ');
  fid = fopen('tfunrun.bat','w');
    fprintf(fid,'%s\r\n','tfun.exe');
    fprintf(fid,'%s\r\n','done.exe ');
  fclose(fid);

   % run the batch file        
   [status , result] = system('tfunrun.bat')
...
%% now we can plot
fid=fopen('timfun_select.dat');
   D=textscan(fid,' %f %f %f %f');
fclose(fid);

t=D{1};src1=D{2};src2=D{3};src_sum=D{4};

figure(1)
   plot(t,src1,'b',t,src2,'g',t,src_sum,'r')

% set x axis limits 
xlim([starttime-tdur  (60*trdist)+tdur])
xlabel('Time (s)')   
ylabel('Moment Rate (Nm/s)')
 
  legend(['Source ' num2str(srcA)],['Source ' num2str(srcB)],['Sum ' num2str(srcA) '+' num2str(srcB)]);

cd ..

% make an isl file with the src numbers

fid = fopen('sel_nnls.isl','w');
    fprintf(fid,'%u ',srcA);
    fprintf(fid,'%u\r\n',srcB);
fclose(fid);
...
if (get(hObject,'Value') == get(hObject,'Max'))
        ...
        set(handles.strike2,'String',strike1);
        set(handles.dip2,'String', dip1);
        set(handles.rake2,'String',rake1);
