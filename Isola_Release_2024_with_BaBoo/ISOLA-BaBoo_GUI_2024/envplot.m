...
%%%%%%%%%%%%%%%%%%%%%%%%  check if source is under the last interface...!!we need to add
%%%%%%%%%%%%%%%%%%%%%%%%  another layer then .........

disp(['Found the last interface at ' num2str(model_depths(nlayers)) '  km'])
disp(['Source depth is ' num2str(depth) '  km'])

if depth >= model_depths(nlayers)

    disp('Source depth is deeper than the last interface. Adding a 5-km thick layer in crustal.dat.')
...
% % and Mw
text(0.,.95,['Selected source no : ' num2str(selectedsrc)],'FontSize',14)
text(0.,.65,['s/d/r : ' num2str(strike) '/' num2str(dip) '/' num2str(rake)],'FontSize',14)
text(0.,.35,['Mw : ' num2str(Mw,'%4.2f')],'FontSize',14)
axis off

%%
ah1=subplot(3,1,2);
currentPos = get(ah1,'Position');
 
for i=1:length(srcpos)
  bb([sdr1(i) dip1(i) rake1(i)], srcpos(i), cor(i), 0.4, 0, 'r')
end
...
ah2=subplot(3,1,3);
 ...
 ylabel('Correlation');
 grid on
 xlim(ah2,[0 srcpos(end)+1])
 
 % remove 0 and last label
 xticklabels = get(ah2, 'XTickLabel');
 xticklabels{1} = ''; xticklabels{end} = '';
 set(ah2, 'XTickLabel', xticklabels);
...
xlim(ah1,xx)
set(ah1, 'XTickLabel', xticklabels);
...
%  grid on
% 
%  v1=axis;axis([0.7 srcpos(end)+0.3 v1(3) v1(4)]);
% 
%  get(gca,'xtick');
%  get(gca,'xticklabel');
...
% cor
% for i=1:length(srcpos)
%   bb([sdr1(i) dip1(i) rake1(i)], srcpos(i), cor(i), 0.2, 0, 'r')
% end
% 
...
% 
% v=axis
% axis([0.7 srcpos(end)+0.3  v1(3) v(4)]);
