...
% beta= [A{:,2}];
% nsources=20;   % NEW ?????? should be read as paramter of teh function
% CC = jet(nsources);
% colormap(CC);
%% plot the net first 
hudsonnet(1,1,1);
...
%convert to Catresian x=1(N),y=2(E),z=3(down)  Aki and Richards
 ...
 m=[moment_11,moment_12,moment_13;...
    ...
    strike = angles_all(1,1)     %in degrees
    dip    = angles_all(1,2)
    rake   = angles_all(1,3)
...
fprintf(fid2, '%6.0f ', strike);
fprintf(fid2, '%6.0f ', dip);
fprintf(fid2, '%6.0f ', rake);
fprintf(fid2, '%6.2f ', Mw);
fprintf(fid2, '%+.6e ', Mo);
fprintf(fid2,'\n');
% fclose(fid2);
%copyfile median_others.dat '.\boo_plots'




ipos2=ipos .^2 .*5; % ????????????????cotoje?
