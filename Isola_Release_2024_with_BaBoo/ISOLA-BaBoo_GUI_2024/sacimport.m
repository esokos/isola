function varargout = sacimport(varargin)
% SACIMPORT M-file for sacimport.fig
%      SACIMPORT, by itself, creates a new SACIMPORT or raises the existing
%      singleton*.
%
%      H = SACIMPORT returns the handle to a new SACIMPORT or the handle to
%      the existing singleton*.
%
%      SACIMPORT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SACIMPORT.M with the given input arguments.
%
%      SACIMPORT('Property','Value',...) creates a new SACIMPORT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sacimport_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sacimport_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sacimport

% Last Modified by GUIDE v2.5 07-Dec-2005 23:40:11
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @sacimport_OpeningFcn, ...
                   'gui_OutputFcn',  @sacimport_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before sacimport is made visible.
function sacimport_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sacimport (see VARARGIN)

% Choose default command line output for sacimport
...
% UIWAIT makes sacimport wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = sacimport_OutputFcn(hObject, eventdata, handles)
...
disp('This is sacimport 03/03/2012');
...
if compname(3)~='E'
    warndlg('This doesn''t look like EW component file. SAC header KCMPNM value doesn''t end with E. This is just a warning.If you are sure about component orientation you can go on','!! Warning !!')
...
handles.maxewsamples=maxewsamples;
guidata(hObject,handles)
...
if compname(3)~='N'
    warndlg('This doesn''t look like NS component file. SAC header KCMPNM value doesn''t end with N. This is just a warning.If you are sure about component orientation you can go on','!! Warning !!')
    set(handles.nsname,'String',[staname ' ' compname])
    set(handles.nsname,'ForegroundColor','r')
    set(handles.nsname,'BackgroundColor','y')
else
    set(handles.nsname,'String',[staname ' ' compname])
    set(handles.nsname,'ForegroundColor','k')
...
handles.mainpath=mainpath;
%path1
handles.maxnssamples=maxnssamples;
...
if compname(3)~='Z'
    warndlg('This doesn''t look like Vertical component file. SAC header KCMPNM value doesn''t end with Z. This is just a warning.If you are sure about component orientation you can go on','!! Warning !!')
...
maxewsamples=handles.maxewsamples;
maxnssamples=handles.maxnssamples;


if maxnssamples ~=  maxewsamples || maxnssamples ~=  maxversamples
   
    errordlg('It seems that the three traces have different length. Use cut to trim extra length and make lengths equal.','Error');
...
% stname=station_name(1:strfind(station_name,' ')-1)
% handles.stanameonly=staname;
...
% stime_filename=[stname 'stime' '.isl']
% start_time=strrep(strrep(strrep(strrep(get(handles.nstime,'String'),':',' '),'-',' '),')',' '),'(',' ')
% %%%
% disp(['Saving file start time info in file   ' stime_filename])
% fid = fopen(stime_filename,'w');
...
disp(['Select up to point ' num2str(fix(cutpoint1/dt))])
%%% 
ew=ew(1:fix(cutpoint1/dt));   
ns=ns(1:fix(cutpoint1/dt));   
ver=ver(1:fix(cutpoint1/dt));  
% 
tim=(1:1:length(ew));
% 
