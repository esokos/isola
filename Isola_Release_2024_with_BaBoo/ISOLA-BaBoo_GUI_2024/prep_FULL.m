...
% set up plotting as 1 figure with subplots per station
figure(1)
subplot1(numstat,2)
...
%  plot(onecov(100,:)) 
%  title(['corr  row 100 station' num2str(i)] )
%end

%% previous code for 1 station
% if (i==selstnum)
% figure 
...
%%
   ...
   subplot(numstat,2,(i*2-1))

   imagesc(onecov,'CDataMapping','scaled'); colorbar; title (['  COVA before inversion at station  ' statcodes{i}])
...
% end
%% 
   subplot(numstat,2,i*2)

   plot(TDATplot); title(['standardized blue, non-stand red at station ' statcodes{i}]);
   hold on
   plot(DATplot,'r')
