...
% aNN(nus1:nus2,nus1:nus2)=autocov(N,nus2);
% aEE(nus1:nus2,nus1:nus2)=autocov(E,nus2); 
% aZZ(nus1:nus2,nus1:nus2)=autocov(Z,nus2); % covariance matrices
...
% aNN=aNN .* hbase; Pelop no 5 snese vypocet bez hbase ve vsech slozkach
% aEE=aEE .* hbase;
% aZZ=aZZ .* hbase;
%  
% aNE=aNE .* hbase; %nelze delat hbase  zde kdyz se neudela pro nn,ee,zz
% aNZ=aNZ .* hbase;
% aEN=aEN .* hbase;
% aEZ=aEZ .* hbase;
% aZN=aZN .* hbase;
...
DAT = [Nobs' Eobs' Zobs'];
%Symmetrization
%onestat=(onestat + onestat')./2.; % is  symmetrization NEEDED 
