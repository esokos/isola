function deg_grid(or_lat,lat_step,nlat,or_lon,lon_step,nlon,depth) 
% or_lat,lat_step,nlat,or_lon,lon_step,nlon,depth
% latitude of origin, step for lat (degrees), no of points in latitude
% same for longitude
% depth
...
eventdate='20010101';
             ...
             filename1='allsrc.dat';
             %%%% open file
             fid2 = fopen(filename1,'w');
             
             filename2='allsrcdeg.dat';
             %%%% open file
             fid3 = fopen(filename2,'w');
...
for j=1:nlon 
    for i=1:nlat
        ...
        y(j,i)=lat_step*(i-1)+or_lat;
        x(j,i)=lon_step*(j-1)+or_lon;
        z(j,i)=depth;
        [s(j,i),a12] = vdist(or_lat,or_lon,y(j,i),x(j,i));
...
%     staazim(j,i)=azimuth(or_lat,or_lon,y(j,i),x(j,i),grs80.geoid);
%     epidist(j,i)=distdim(distance(or_lat,or_lon,y(j,i),x(j,i),grs80.geoid),'km','km');
%        
        ...
        if isnan(s(j,i))
            ...
            [Ysource(j,i),Xsource(j,i)] = pol2cart(deg2rad(a12),(s(j,i)/1000));
           
        end
        
             filename=['src' num2str(sourceindex+i,'%02d') '.dat'];
             %%%% open file
             fid = fopen(filename,'w');
             fprintf(fid,'%s\r\n',' Source parameters');
             fprintf(fid,'%s\r\n',' x(N>0,km),y(E>0,km),z(km),magnitude,date');
             fprintf(fid,'%10.4f%10.4f%10.4f%10.4f  %c%s%c\r\n',Ysource(j,i),Xsource(j,i),z(j,i),5, '''', eventdate, '''');
             fclose(fid);
        
             fprintf(fid2,'%10.4f%10.4f%10.4f\r\n',Ysource(j,i),Xsource(j,i),z(j,i));
...
% 
% figure(3)
%    m_proj('Mercator','long',[min(min(x))-0.1 max(max(x))+0.1],'lat',[min(min(y))-0.1 max(max(y))+0.1]);
%    m_gshhs_h('color','k');
%    m_grid('box','fancy','tickdir','out');
%    %%%plot epicenter
%    m_line(or_lon,or_lat,'marker','p','markersize',17,'color','b','MarkerFaceColor','b');
% 
%              for j=1:nlon
%                 for i=1:nlat
%                     m_line(x(j,i),y(j,i),'marker','square','markersize',5,'color','r');
