...
% Last Modified by GUIDE v2.5 01-Dec-2022 13:53:31
...
%% check if we have some selected source pair from NNLS  30/11/2022
if isfile('sel_nnls.isl')
 
fid = fopen('sel_nnls.isl','r');
   D=textscan(fid,' %u %u');
fclose(fid);

src1=D{1};src2=D{2};
% now set src1/2 popups to proper src !
set(handles.nosourcepopup1,'Value',src1);
set(handles.nosourcepopup2,'Value',src2);
...
if strcmp(src1,"") || strcmp(src2,"")
    
    errordlg('Select two sources','Error');
    return
else
...
if strcmp(src1,"")  
    ...
    errordlg('Select a source','Error');
    return
else
...
dt=str2num(handles.linetmp4); 
newdt=str2double(get(handles.trdist,'String')); 

 

if newdt < dt
     mess=['Spacing value cannot be smaller than dt, (=' num2str(dt) ')'];
     errordlg(mess,'Error');
     set(handles.trdist,'String',dt)
else
...
%%  we need to alter inv222.dat file to reflect the selected tdur !!
% check that inv222.dat exist
if ispc
  if ~isfile('.\timefunc\inv222.dat')
    errordlg('File inv222.dat not found. Run Calculate.','File Error');
  else
  end
else
  if ~isfile('./timefunc/inv222.dat')
    errordlg('File inv222.dat not found. Run Calculate.','File Error');
  else
  end
end
%% change inv222.dat file to reflect the selected duration (tdur)
cd timefunc
   
   movefile inv222.dat inv222.tmp
   
   [fid1, msg1] = fopen('inv222.tmp', 'r');
      [fid2, msg2] = fopen('inv222.dat', 'w');
      
   % write first line in new inv222.dat
      if ispc
        fprintf(fid2,'%u  %u\r\n',tdur,60);
      else
        fprintf(fid2,'%u  %u\n',tdur,60);  
      end
     % read and write lines will be 120
     tline=fgetl(fid1);
...
% inv222.dat is now updated we must run timfuncas,1,2,3
%%
system('timfuncas.exe')
system('timfuncas1.exe')
system('timfuncas2.exe')
...
% --- Executes on button press in cpfiles.
function cpfiles_Callback(hObject, eventdata, handles)
% hObject    handle to cpfiles (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

%% ask for copying *fil.dat and *syn.dat into Invert for plotting
answer = questdlg('Should i copy *fil.dat and *syn.dat files to invert folder for plotting?', ...
	'Copy files', ...
	'Yes','No','No');
% Handle response
switch answer
    case 'Yes'
        disp('Copying files')
        [status,message,messageId] = copyfile('.\timefunc\*fil.dat', '.\invert\') 
        
        [status,message,messageId] = copyfile('.\timefunc\*syn.dat', '.\invert\') 
        
        f = msgbox('Operation Completed');
        
    case 'No'
        disp('Abort copying files')
