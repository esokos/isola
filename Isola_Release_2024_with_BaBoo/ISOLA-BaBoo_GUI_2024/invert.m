...
%%
disp('This is invert 28/11/2022');
...
%% read ISOLA defaults
[gmt_ver,psview,npts,~,cova] = readisolacfg;
dtres=tl/npts;
...
if ispc
  fid = fopen('.\invert\inpinv.dat','w');
    ...
    fprintf(fid,'%s\r\n','    number of subevents to be searched (isubmax), ');
    fprintf(fid,'%i\r\n',nsubevents);
...
%% check if time search is compatible with triangle duration 28/11/2022

if istype ==2 % triangle
    
    halfdur=t0/2
    endtime=etime 
    if halfdur >= endtime
       f = warndlg('Your time search end is lower than the half duration of triangle; Increase the end time','Error');
       waitfor(f);
    else
    end
...
%% 
        ...
        %%  now run inversion 
        %   RUN the batch files
        %   changes to include COVA
        %button = questdlg('Run Inversion ?','Inversion Methods','Yes NO COVA','Yes WITH COVA','No','Use CSPS','Yes NO COVA');

        button = questdlg('Run Inversion ?','Inversion Methods','Yes NO COVA','Yes WITH COVA','Use CSPS','Yes NO COVA');
        
        if strcmp(button,'Yes NO COVA')

            % extra question if we need standard NO COVA or nocova-BaBOO
            button2 = questdlg('Select method ?','Inversion Methods','Standard NO COVA','NO COVA BaBOO','Standard NO COVA');

               if strcmp(button2,'Standard NO COVA')
                     disp('Running NO COVA inversion')
                     % update cova.sel
                     fid = fopen('cova.sel','w');
                          fprintf(fid,'%s\r\n','standard');
                     fclose(fid);  
                    ...
                    if ispc 
                      ...
                      fid = fopen('runisola_nocova.bat','w');
                     ...
                     disp('Linux ')
                       fid = fopen('runisola_nocova.sh','w');
                        fprintf(fid,'%s\n','#!/bin/bash');
                        fprintf(fid,'%s\n','             ');
                        fprintf(fid,'%s\n','isola_nocova.exe');
                        fprintf(fid,'%s\n','norm.exe');
                     ...
                     %  wait here for fortran part to end
                      ...
                      cd ..   % return to ISOLA run folder 
                      pwd
               elseif strcmp(button2,'NO COVA BaBOO')
                  % here we call bootstrapgui

                      bootstrapgui
               end


        elseif strcmp(button,'Yes WITH COVA')
            
          disp('Running inversion WITH COVA')
             disp('Be sure that before this you made a run without COVA.')
          warndlg('Be sure that before this you made a run without COVA','Warning');
          
           % select the type of COVA we need
           % buttoncova = questdlg('Select type of COVA','COVA menu','COMPVAR','FULL','AXCF' ,'COMPVAR');
             buttoncova = questdlg('Select type of COVA','COVA menu','Variances','Toeplitz','Non-Toeplitz','Toeplitz');
           
           %%
           % if strcmp(buttoncova,'Variances')
           %        covatype='Variances';
           % elseif strcmp(buttoncova,'Toeplitz')
           %        covatype='Toeplitz';
           % elseif strcmp(buttoncova,'Non-Toeplitz')
           %        covatype='Non-Toeplitz';
           % end
           % 

           % update the cova.sel
           fid = fopen('cova.sel','w');
             fprintf(fid,'%s\r\n',['with cova ' buttoncova]);
           fclose(fid);  

           %% go in invert
           cd invert
           
           % Run proper Matlab code, in invert folder,  based on selection  first we run the matlab code that will make the hinv.bin file
           if strcmp(buttoncova,'Variances')
               disp('Running prep_COMPVAR')
                  prep_COMPVAR 
           elseif strcmp(buttoncova,'Toeplitz')
                  disp('Running prep_FULL')
                  prep_FULL 
           elseif strcmp(buttoncova,'Non-Toeplitz')
                  % prep_AXCF
                  disp('Running prep_FULLMOV')
                  prep_FULLMOV
           end
           
           %% now run the fortran part ... if all is ok we are still in
           % invert folder
...
if invpal == 1
    if gmt_ver==6
        vstring=['makecpt -C' gmtpal ' -T0/' num2str(maxvar+0.05) ' -I corcon.dat  -S0.1+d  > cr.cpt'];
    else
        vstring=['makecpt -C' gmtpal ' -T0/' num2str(maxvar+0.05) '/' num2str(cptstep) ' -I  > cr.cpt'];
    end
else
    if gmt_ver==6
        vstring=['makecpt -C' gmtpal ' -T0/' num2str(maxvar+0.05) '   corcon.dat  -S0.1+d  > cr.cpt'];
    else
...
if ispc
   ...
   %
   % change rename with move to force overwrite 28/11/2022
   fprintf(fid,'%s\r\n',['move /y ..\output\' psname(1:6) '.png  ..\output\' eventid '_' psname(1:6) '.png']);
...
elseif conplane==4  %% new type of plot
disp('This is a new type of plot not ready yet   ...!!')  
...
msgbox(["Subevent times that we search and report in the results refer to the beginning of the triangle (not to its maximum).";" For the triangle of duration T, these times will be approximately T/2 before the times reported for Delta function."],"Warning");
...
status=get(handles.freqcheck,'value');

fid=fopen('freqchk.isl','w');
