c      program ANGGRADONE
c VERSION: August, 2020
c
c This version containes a correction introduced into the code, to avoid ommiting some stations
c in the output MYPOL.DAT, see line 708
...
c
c As compared with the program ANGGRAD only one, the fastes wave is given in output for each station.!!!!!!!!
c
c In the case of the crustal model, bounded by MOHO, code supports two cases:
c
c a) Vp is constant below Moho (Pn head wave exists)
c The MOHO layer must be specified as a constant-velocity layer,
c i.e., the line for the MOHO looks like 
c 43.0  8.2  8.2
c where 43 is the Moho depth, and 8.2 is Vp below Moho the velocity at the bottom of the MOHO layer equals to this value.
c One fictious layer must be added below Moho to specify the depth of the MOHO layer bottom, i.e.
c 60.0  8.2  8.2   ... (the fictious layer)
c Note that all four velocity values at the two last model layer must be identical, equal to  the velocity under MOHO.
c The top of the fictious layer (e.g. 60.) must be smaller than 100.  
c
c b) Vp increases below Moho
c like, e.g.
c 43.0 8.2 8.3 
c i.e. the velocity at the bottom of the MOHO layer equals to 8.3.
c One fictious layer must be added below Moho, to specify the gradient below Moho and the depth of the MOHO layer bottom
c 60.0 8.3 8.3 ... (the fictious layer)
c Note that the velocity at the bottom of the MOHO layer and both velocities for the fictious layer must be identical.
...
c is the layer homogeneous ?
      if(w(is)-v(is).gt.0.005)go to 432
...
c
      ir=1
      vr=v(1)
c vr = velocity at the depth of the source
c
      write(6,56)
...
c
   74 continue 
      ...
      if(abs(cosuhs).lt..00001)go to 302
...
cc cc     write(80,801)
cc cc    * stcode,iazi,ifix(adirp),pol,epidis,tdirp ! direct P upwards
c      
cc           go to 2000
      go to 239
c
...
c
c  the ray has no minimum (radiat. upwards)
...
c
  204 continue
c zde prisp. od minima od vrstvy, lezici nekde pod vrstvou se zdrojem
      jaj=i
            jobrat=jaj
...
c To overcome this failure we take for such a case ray's approximation by 
c the boundary ray. We use this approximation if the difference between the epicentral
...
c
c there is applied the correction (between lines 731 and 750)
c
c
      go to 216
...
c
  150 continue
      sinuhs=par
      cosuhs=sqrt(abs(1.-sinuhs**2))
      if(abs(cosuhs).lt..00001)go to 300
      tanuhs=sinuhs/cosuhs
c
      angle=atan(tanuhs)*57.2958
      go to 301
...
ccccc
c
      if(iii.eq.1)go to 291
c
c here are saved values for the downward refract. wave
      iii=1
      tdirpd=ttim                  
      adirpd=angle
	iadirpd=ifix(adirpd)			! New jz Aug 2015
...
c
  291 continue
c  compare with values of head waves
      if(ttim.gt.tdirpd)go to 44
      tdirpd=ttim                  
      adirpd=angle
	iadirpd=ifix(adirpd)			! New jz Aug 2015
 	if(iadirpd.eq.90) iadirpd=91 	! New
...
c
c
      if(tdirpd.lt.thead1.and.tdirpd.lt.tdirp)write(80,803)
     * stcode,iazi,iadirpd,pol,epidis,tdirpd,jjobrat ! direct P downwards
      if(tdirpd.lt.thead.and.tdirpd.lt.tdirp)go to 2000
c
      if(tdirp.lt.thead1.and.tdirp.lt.tdirpd)write(80,801)
     * stcode,iazi,iadirp,pol,epidis,tdirp
      if(tdirp.lt.thead.and.tdirp.lt.tdirpd)go to 2000
c
      if(thead1.lt.tdirp.and.thead1.lt.tdirpd)write(80,801)
