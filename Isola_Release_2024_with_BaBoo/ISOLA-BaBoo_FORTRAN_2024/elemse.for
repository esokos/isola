...
c Delta centered at t=0
c Triangle centered at T/2 
...
c
      ...
      parameter   (nsp=1,ncp=200,nrp=21,ntp=1024,nrtp=nrp*ntp)
      parameter   (nrtp2=nrtp*6)


      integer     iwk(ntp),jf,ir,it,nc,ns,nr,nfreq,ikmax,mm,nt
      real        tl,xl,uconv,
     3            dfreq,freq,
     4            pi,pi2,aw,ck,cc,
     5            pas,
     7            t1
c     real        a(6)
      complex*16  ux(ntp,nrp,6),uy(ntp,nrp,6),uz(ntp,nrp,6),omega,
...
c     &                     nrtp2*(0.,0.)/
 	...
 	do itim=1,ntim  			   ! thn THIS initiation solves the problem
 	ux(itim,ir,it)=(0.,0.)
 	uy(itim,ir,it)=(0.,0.)
 	uz(itim,ir,it)=(0.,0.)
      enddo
...
c     open (12,form='unformatted',file='gr.hes') ! Powerstation
      ...
      open (130,form='formatted',file='spefct.dat')
cc    open(350,form='formatted',file='elemsef.dat') ! elem seis formated
c      open(300,form='unformatted',file='elemse.dat') ! Powerstation
      open(300,form='unformatted',file='elemse.dat',
     *     access='stream',status='replace') ! Ifort (output)
...
ccccccccccccccccccccccccccc
      mm=nint(log(real(ntim))/log(2.)) ! nint, not int
...
c      if(ics.eq.7) us=us*exp(-ai*omega*10.)    !! delta NOT shifted at all 
      if(ics.eq.4) us=us*exp(-ai*omega*(t0/2.))    !triangle centered at T/2
