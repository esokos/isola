      program ENERGY_svsh_v02b
c v02 reads konstant tstar for all stations
c differs from v02 on line 159 and increases energy by a few percent 
...
c      character *17 statfil1, statfil2,statfil3
      ...
      dimension xdist(100),xazim(100),xtake(100),spredp(100)
      dimension tstarst(100)
      dimension pobs(100),scom(100),pdursyn(100),pdurobs(100)
      dimension ep(100),es(100),devi(100)
      open(105,file='eneinp0_v02.dat')  
      open(110,file='eneinp_v02.dat')  
      open(200,file='eneout.dat')
      open(201,file='eneout1.dat')
      open(210,file='eneout2.dat')
      open(220,file='eneout3.dat') !output: rad. pat. squared [P, S, SH, SV]
...
cccccccccccccccccccccccc
      tstarst=tstarconst       ! FIXED if later read for each station, then updated
      write(*,*) tstarst
cccccccccccccccccccccccc
...
ccc      ilim2=ifix((pobs(ir)+pdursyn(ir))/dt) ! almost same as with 20s
      ilim3=ifix((scom(ir)-15.)/dt)    ! S window starts 5 second before prescribed  S arrival  
...
c	   pattp2=pattp**2.  !   
     ...
     *     0.5*sin(rak)*sin(2.*dip)*sin(ang)*sin(2.*(azi-str))
      ...
      write(220,'(1x,i5,1x,a5,4(1x,e15.6))')
     & ir,statname(ir),pattp2,patts2,pattsh2,pattsv2    ! squared rad pat
