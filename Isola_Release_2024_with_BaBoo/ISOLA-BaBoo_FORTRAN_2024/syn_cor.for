...
c      use msimsl ! for random number generation (RNSET, RNUN): sunstituted by Num. Recipes
      ...
      dimension bb(3,1024),xpozice(1024)
	  DIMENSION XAUX(1024),TREND(1024)
      dimension amax(3)
...
c      write(*,*) 'how many seconds of noise to create?' ! later add among data prepared by GUI
c      read(*,*) xsecon
c     similarly to this last read, here we could read xnoise std. deviation as station dependent	  
      xsecon=float(1024)*dt 
      endif	
