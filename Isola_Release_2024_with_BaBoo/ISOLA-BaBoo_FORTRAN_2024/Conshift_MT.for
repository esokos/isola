...
c 2022: 
c ntim=1024
c open unformatted files for Ifort
c int-->nint

c      Reading only time shift from mechan.dat
c      Reading MT components from moment_tensor.dat; MT is in Nm (not normalized to 1)
c	 MT is either in Cartesian (1) or spherical coordinates (2)
c      Cartesian MT must be ordered like this: Mxx,Mxy,Mxz,Myy,Myz,Mzz
c      (caution - this is different order than in sile5!!)
c      Spherical coordinate MT must be ordered like this: Mrr,Mtt,Mpp,Mrt,Mrp,Mtp
 
c
ccc       fixed  ntim ! mm defined via ntim  
...
c   J.Zahradnik, 1997  and 1998  and 2015 (MT)
ccccc
...
c          output in 3 files according to STATIONS!!!
      ...
      real         lat0,lon0
      parameter   (nsp=1,ncp=200,nrp=21,ntp=1024,nrtp=nrp*ntp) ! ntp ... FIXED !! see also mm=
...
c     open (12,form='unformatted',file='gr.hes') ! Powerstation
      open (12,form='unformatted',file='gr.hes',access='stream') ! Ifort (input)  
...
c	read(301,'(f4.1)') t0	! old
	...
	open(401,file='moment_tensor.dat') !	
c       Cartesian MT components  ordered like this: Mxx,Mxy,Mxz,Myy,Myz,Mzz
	read(401,*) key_mt
	if(key_mt.eq.1) then 
      read(401,*) x1,x2,x3,x4,x5,x6
	write(*,*) 'MT in Cartesian coordinates'
      else
c       Spherical  components  ordered like this: Mrr,Mtt,Mpp,Mrt,Mrp,Mtp
	read(401,*) xrr,xtt,xpp,xrt,xrp,xtp
	write(*,*) 'MT in spherical coordinates'
      x1=xtt
      x4=xpp
      x6=xrr
      x2=-xtp
...
c     convertion depth -> thickness
  ...
  91    continue
      ...
      call cmoment_mt (mu(is),x1,x2,x3,x4,x5,x6,a(1,is))
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	stop
      ...
      subroutine cmoment_mt (mu,x1,x2,x3,x4,x5,x6,a)
...
c       Cartesian MT components  ordered like this: Mxx,Mxy,Mxz,Myy,Myz,Mzz
c	x1 =-(sd*cl*s2p + s2d*sl*sp*sp)*xmoment
c	x2 = (sd*cl*c2p + s2d*sl*s2p/2.)*xmoment
c	x3 =-(cd*cl*cp  + c2d*sl*sp)*xmoment
c	x4 = (sd*cl*s2p - s2d*sl*cp*cp)*xmoment
c	x5 =-(cd*cl*sp  - c2d*sl*cp)*xmoment
c	x6 =             (s2d*sl)*xmoment
c
c 100% ISO
