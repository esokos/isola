...
c Author: J. Zahradnik 2017, 2024
      ...
      dimension x(204800),y(204800),z(204800),tim(204800)
      dimension xr(204800),xt(204800),xsv(204800),xsh(204800)
      dimension xx(204800),yy(204800),zz(204800),x3com(204800)
      dimension aspx(102400),aspy(102400),aspz(102400) ! amplitude spectra of velocity 
      dimension aspx2(102400),aspy2(102400),aspz2(102400)
      dimension asp2(102400),asp2smoo(102400)  
      dimension asp3com(102400),asp3com2(102400) ! amplitude spectrum of absolute velocity, and its square 
...
c      sigma=sigma_seismic ! Only Rad.pat of P depend on sigma (and gamma)	  
       sigma=sigma_source   ! Only Rad.pat of P depend on sigma (and gamma)
c      sigma=0.25
...
cccccccccccccccccccccccc
      ...
      NI=14 ! (assumes 16384 points)     !!!!!!!!!! NEW
      NT=2**NI  ! 16384, i.e. TWL = 2048 seconds 
	  dt=0.125 ! TWL/(float(NT)) !  !!!!!!!!!!!NEW !!
      NP=NT     
      NT2=NT/2
      NTM=NT2+1         
      NTT=NT+2
      DF=1./(DT*FLOAT(NT))
c      ntm=0.7/df               !!!temporary 
...
c Finite frequency bandwidth correction
 	  fmax= 0.5 ! 1./(2.*dt)   !!!!!! This is Nyquist frequency   
...
c	    write(*,*) x
        ...
        call filtless_16384(dt,x,aspx) ! aspx = amplitude spectrum of velocity (from f=0 to Nyquist)
        call filtless_16384(dt,y,aspy)
c       call filtless_16384(dt,z,aspz)
...
cc    Using asp2 and visualizing integrand   
		 ...
		 freq=float(i-1)*df
         contrib= exp(2.*pi*freq*tstarst(ir)) * asp2(i) ! * df visualize without df!!
         spint=spint + contrib 
        write(230,'(2(1x,e13.6))') freq, contrib
        enddo
		spint=spint*df ! TEMP added because I removed df for plotting
c        write (230,*) '*'		
...
c**********************************************************      
     ...
     *     0.5*sin(rak)*sin(2.*dip)*sin(ang)*sin(2.*(azi-str))
      ...
      write(216,'(1x,i5,1x,a5,4(1x,e15.6))')      ! 100% DC, not squared (with sign)
     &ir,statname(ir),pattp, patts, pattsh, pattsv      ! non-squared
...
c      write(216,'(1x,i5,1x,a5,4(1x,e15.6))')           ! non-DC, not squared (with sign)
c     &               ir,statname(ir),pattp,pattsnew,pattsh,pattsv
...
c ....Averaging radiation patterns squared (continual) .........
