      program NEWASPO9WISH_1024Intel

c  WISH = Waveform Inversion With Shifts (without envelopes)

c 2022
C Intel
c see open of unformatted files, e.g.  in elesyn!!
c Can be used only with Green functions and elemse files from Intel (or Gfort)
c Modifications  in arrays w, transform into 1D arrays ... w(1:1024... (done but not necessary)
c Konverze 3D a 4D poli (asi zbytecne) jsou v programech main, elesyn, prepsyn a maniobs
c Major mistake was missing normalization w=w*1.e10 without which Power worked OK but Intel produced mistakes fue to e-38 ...
...
c REFERENCE: 
c Zahradnik, J., and E. Sokos (2018): Waveform Envelope Inversion - A New Tool of ISOLA Software.
c Seismological Research Letters. doi: 10.1785/0220170161
...
c      double precision xmo,sum1,sum2,asx,asy,asz
	  dimension aox(1024,21),aoy(1024,21),aoz(1024,21)
      dimension aoxx(1024),aoyy(1024),aozz(1024)
      dimension asx(1024,21),asy(1024,21),asz(1024,21)
c      dimension asxx(1024),asyy(1024),aszz(1024)
      dimension sx(1024,21,3)
      dimension  x(1024,21,3)
      dimension y1(1024,21,3),y2(1024,21,3)
...
c      Depths (needed for inv1.dat plot) are taken from the GUI!
     ...
     *                 ff1(21),ff2(21),ff3(21),ff4(21),dt
      ...
      write(*,*)
	  write(*,*) 'This is NEWaspo9WISH_1024Intel'
	  write(*,*)
...
c     FIXED OPTIONS
      ntim=1024 ! number of time samples  
	  nmom=6
      pi=3.1415927
...
c      enddo	  
      ...
      if(keysdr.eq.0) then         ! Just to calculate nall
      nall=0  ! 
...
c *******************************************************************
       ...
       call maniobs_bezobal_1024(x,y1,y2,rr)    
c       write(*,*)'maniobs x,y1,y2',x(100,4,3),y1(100,4,3),y2(100,4,3) 
...
c Output envelope y1, output ampl.sp y2. BOTH y1 and y2 with ntim points
c Power of the data rr 

      do i=1,ntim ! nump (isola plotting needs all)
      do ir=1,nr
      aox(i,ir)=0.
      aoy(i,ir)=0.
      aoz(i,ir)=0.
...
c      write(*,*) filename
	  call elesyn_1024Intel(filename,w) ! w~e-20
	  
C---------------------------------------------------------------------      
	  w=w * 1.e10 ! NEW  to prevent underflow  ! THIS IS A KEY FEATURE 
C---------------------------------------------------------------------      
	  
c	  write(*,*) 'prvni after 1e10; isour and w'
c	  write(*,*) isour,w(100,4,3,2)
...
c *************************************************************************
c        GS over  s/d/r angles in  THREE NESTED LOOPS 
...
c *************************************************************************
	  ...
	  indexopt=1 ! NEW  ... INTEL si zada na radce 849 (highly unlikely that needed)
  	  
      if(keysdr.eq.1) then
      istfr=10          ! NEW For Intel it was error, istfr was not defined CAUTION; Powerstation was fine
	  idifr=10          ! NEW .............. KEY FEATURE ........................
	  irafr=10          ! NEW
...
c      write(*,*) 'afix',afix
c  We generate synthetics for shiftini = =shiftOT+shiftplus
...
c      write(*,*) ishf
	  call subevnt15_1024(afix,w,ishf,sx)     ! sx~e-25
c	  write(*,*) sx(40,4,3)

c      w is NOT re-written, sx is created
c      sx is  synth  filtered displacement(incl. focmech)

      call prepsyn_bezobal_1024(sx,y1,y2) ! envelope and ampl.sp. of the synth displacement filtered	
c 	  write(*,*) 'prvni '
c	  write(*,*) 'after prepsyn',sx(40,4,3),y1(40,4,3),y2(40,4,3)
...
c      write(*,*) ir,smax(ir),snorm
	  do jf=1,nump
      asx(jf,ir)=asx(jf,ir)/snorm      ! norm ~ 1.0 , and no norm ~ e-20 * e10 = e-10 
...
c In the normalized mode it is not needed
      ...
      do ir=1,nr
      do jf=1,nump
      asx(jf,ir)=asx(jf,ir)*xmo       !  no norm: ~ e-10 * e16 = e+6; norm 1 * e16 = e16 ;      
...
ccccccccccccccccccccccccccccccccccc
      ...
      do ir=1,nr ! 
	  call mycorlag2(aox(1:1024,ir),asx(1:1024,ir),1024,lag,ccx)      ! aox, asx different ORDER!
	  call mycorlag2(aoy(1:1024,ir),asy(1:1024,ir),1024,lag,ccy)
	  call mycorlag2(aoz(1:1024,ir),asz(1:1024,ir),1024,lag,ccz)
c          do iii=1,ntim
c		  aoxx(iii)=aox(iii,ir)          !  not needed
c		  asxx(iii)=asx(iii,ir)
c		  aoyy(iii)=aoy(iii,ir)
c		  asyy(iii)=asy(iii,ir)
c		  aozz(iii)=aoz(iii,ir)
c		  aszz(iii)=asz(iii,ir)
c		  enddo
c	  call mycorlag2(aoxx,asxx,1024,lag,ccx)
c	  call mycorlag2(aoyy,asyy,1024,lag,ccy)
c	  call mycorlag2(aozz,aszz,1024,lag,ccz)
c	  write(*,*) 'ahoj2'
      ...
      enddo ! over stations
c	  write(*,*) 'last stat:'isour,ncount,lag, ccx,ccy,ccz
...
c AUXILIARY: compensation of the previous multiplication by xmo before seeeking lag
      ...
      do ir=1,nr
      do jf=1,nump
      asx(jf,ir)=asx(jf,ir)/xmo     ! no norm ~e6/e16 ~e-10 ;   norm e16/e16 ~ 1 AS BEFORE
      asy(jf,ir)=asy(jf,ir)/xmo
      asz(jf,ir)=asz(jf,ir)/xmo
      enddo
      enddo
...
c                          	  !  no norm: ~ e-10; norm 1 (both for spectra and envelopes)
...
c *******************************************************
      ...
      sum1=sum1+wax(ir)**2.*(aox(i,ir)*asx(i2x,ir))+       ! no norm e-4 * e-10 = e-14
     *          way(ir)**2.*(aoy(i,ir)*asy(i2y,ir))+ 
     *          waz(ir)**2.*(aoz(i,ir)*asz(i2z,ir))
      sum2=sum2+wax(ir)**2.*(asx(i2x,ir)**2.)+             ! no norm (e-10)**2 = e-20
     ...
     *          waz(ir)**2.*(asz(i2z,ir)**2.)
	  ...
	  xmo=xmo !        no norm   e-14/e-20 ~ e6 

c      write(*,*) 'prvni'
c	  write(*,*) 'sum1,sum2,xmo',sum1,sum2,xmo
      do ir=1,nr
      do i=1,nump
      asx(i,ir)=asx(i,ir)*xmo ! no norm : asx~ e-10 * e6 = e-4
      asy(i,ir)=asy(i,ir)*xmo
      asz(i,ir)=asz(i,ir)*xmo
      enddo
      enddo

 3476 continue	      ! no norm : asx~ e-4;    for norm asx ~ 1, 
...
c ***************************************************
      ...
      sumx=sumx+wax(ir)**2.*(aox(i,ir)-asx(i2x,ir))**2.  ! no norm: e-4 minus e-4; for norm: 1 minus 1
      ...
      error(ncount)=sum/rrori ! here we may have also possibly sum (L1 misfit); rrori knows about norm/no-norm

      var=1.-sum/rrori             ! variance reduction, including weights 
c      write(*,*) 'rrori,var,sum=',rrori,var,sum 
...
c output of all solutions for source isour
      ...
      write(898,'(1x,i6,3(1x,i6),1x,e10.3)')
...
c      write(897,*) 'isour,GSpoint#,corr,nil,DC%,s,d,r,s2,d2,r2'
     ...
     *   istr,idip,irak,istr2,idip2,irak2
	  ...
	  dum0=0.
      write(896,
     *   '(1x,i5,1x,f6.2,1x,e12.5,12(1x,f6.0),1x,f6.1,1x,e12.4)')
...
c      write(*,*) 'druhy'
      filename='elemse'//chr(isour)//'.dat'
      call elesyn_1024Intel(filename,w)

c ------------------------ KEY FEATURE ---------------------------- 
      w=w * 1.e10 ! New to prevent underflow 
c ------------------------ KEY FEATURE ---------------------------- 
...
c      write(*,*) w(100,4,3,2)
      ...
      if(keyinv.eq.0) then
      write(894,*) 
      write(894,*) 'Your assumed origin time = ', shiftOT
	  write(894,*) 'Here is time including optimal shifts:'
	  write(894,*) 'station, x-comp, y-comp, z-comp:'
      do ir=1,nr
     ...
     *	  ilagx(ir)*dt,ilagy(ir)*dt,ilagz(ir)*dt
      enddo
      call subevnt15lag_1024(afix,w,ilagx,ilagy,ilagz,sx) 
      else
      ioptshf=ifix(shiftini/dt) ! possible error of 1 dt 
      call subevnt15_1024(afix,w,ioptshf,sx)
      endif
      
      call prepsyn_bezobal_1024(sx,y1,y2) ! envelope and ampl.sp. of the synth displacement filtered	
c      write(*,*) 'druhy after prepsyn'
c	  write(*,*) 'sx,y1,y2'
c	  write(*,*) sx(100,4,3),y1(100,4,3),y2(100,4,3)
      
	  do i=1,ntim ! not nump (isola plotting needs all even if we plot spectra having less)
      do ir=1,nr
      asx(i,ir)=0.
      asy(i,ir)=0.
      asz(i,ir)=0.
...
c ------------------------ KEY FEATURE ---------------------------- 
      xmo=xmo * 1.e10 ! Compensation for artifical multiplication 
c ------------------------ KEY FEATURE ---------------------------- 
c      write(*,*) 'sum1,sum2,xmo=',sum1,sum2,xmo 
      ...
      keymom=1
      pompom=-pompom
	  write(*,*) 'ERROR in Mo and Mw!!!'
      endif	  
      if(keymom.eq.1) then
	  write(900,*) 'ERROR in Mo and Mw!!!'
      else      
      ...
      write(900,'(1x,i6,1x,e10.3,3(1x,i6))')
...
c     renorm=1. ! this EDIT possible to PLOT non-normalized when normalization allowed  
	  nfile=3000+1*ir  
        do i=1,ntim !!!!!! nump (isola plotting needs all but then we need initialization) 
        if(keyinv.eq.0) then 
        xxx=float(i-1)* dt
        else
        xxx=float(i-1)* df
...
c      include "maniobs_1024.inc"
      include "maniobs_bezobal_1024.inc"
      include "subevnt15_1024.inc"
      include "subevnt15lag_1024.inc"	  
      include "elesyn_1024Intel.inc"
c      include "prepsyn_1024.inc"
      include "prepsyn_bezobal_1024.inc"
      include "genacka.inc"
      include "filter15_1024.inc"
      include "filtmore_1024.inc" 
      include "fcoolr.inc"
