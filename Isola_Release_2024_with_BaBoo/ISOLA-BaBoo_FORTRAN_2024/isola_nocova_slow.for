...
c     !     CAUTION: must be unified with ELEMSE (in Green) !!!!!!!!!!
	  integer, parameter :: NBS=21 ! NBS = max number of stations
	  integer, parameter :: NMUL1= NBO*NBS*3
c	  integer, parameter :: mind=NMUL1*3 ! assumed maximum number of non-zeros of Cd-1      
...
c	  module mcovsparse
c	  integer, allocatable :: ja(:),ia(:)
c      real, allocatable    :: acsr(:)	  
c	  endmodule
	  
	  
      program ISOLA_NOcova_slow

c MUST be compiled in Ifort or Gfort, NOT in Powerstation 
c must contain 'stream' in 'open' for binary files	  

c Does NOT need MKL library!

c     Author: J. Zahradnik 2024	  
...
c	  dimension job(8), acoo(mind) ! = max. number of elements of sparse Cd-1
c     dimension irowind(mind),icolind(mind)


      allocate (ff1(NBS),ff2(NBS),ff3(NBS),ff4(NBS),
     *	       weig(NBS,3))
c      allocate (ja(mind),ia(mind),acsr(mind)) 
...
c      open(210,file='hinv.dat')      ! input: Cd-1 sparse in COO (from my matlab code)
...
c     open(674,file='correl.dat')     ! 2d correlation (see later)
      ...
      write(*,*)
	  write(*,*) 'This is ISOLA_NOcova_slow, version 2024b'
	  write(*,*)
...
c      if(keycova.eq.1) goto 1656 ! go to new version	
...
c 1656 continue 	  
c	  ir=1
c 71    read(210,*,end=81) irowind(ir),icolind(ir),acoo(ir)
c      ir=ir+1
c      if(ir-1.gt.mind) then
c      write(*,*) 'Problem DIM. sparse matrix'
c      STOP
c      endif	  
c      goto 71	  
c 81   nonz=ir-1
c      write(*,*) 'number of non-zero elements of Cdinv= ',nonz  ! number of elements of COO matrix       

c      nsz=ntim*nr*3
c      nsz2=ntim*3
	  
c	  job=0
c	  job(1)=1 ! matrix A from COO (acoo) to CSR (acsr)
c	  job(2)=1 ! one-based indexing for matrix in CSR
c	  job(3)=1 ! one-based indexing for matrix in COO
c     job(5)=  nonz ! see above max number of nonzero 
c      job(6)=0 ! all arrays acsr, ja, ia are filled-in for the output storage
c      !    ja ... so-called 'columns' array in SCR
c      !    ia ... so-called 'rowIndex' array in SCR
c  	  
c      call mkl_scsrcoo(job, nsz, acsr, ja, ia, nonz, 
c     *                 acoo,irowind, icolind, info)
c
c	  write(*,*) 'info= ',info
c	  write(*,*) 'acoo(1)= ',acoo(1)
c	  if(info.ne.0) then
c	  write(*,*) 'Problem with sparse matrix'
c	  STOP
c      endif	
...
c *******************************************************************
      ...
      call elemat_play_NOcova(filename,w,rold,rinv)
...
c ************************************************************************
      ...
      if(keyinv.eq.1)
     *	  call oneinv_play_NOcova(xinv,w,rinv,asave,corr,ish) ! nmom=6 
      if(keyinv.eq.2)
     *	  call oneinv_play_NOcova(xinv,w,rinv,asave,corr,ish) ! nmom=5
      if(keyinv.eq.3)
     *	  call cnsinv_play_NOcova(xinv,w,rinv,asave,corr,ish)
      if(keyinv.eq.4)
...
c      vopt(n)=twovvvv(iselect,isequen,n)
      ...
      aopt(n)=aopt(n)*0.25	 !  MODIF umele snizeni momentu pro VSECHNY SUB !!! "slow"
      enddo
...
c      write(300,*) ' Covariance matrix; corrected for  scaling'
      write(300,*) ' Covariance matrix; NOT corrected for  scaling'
      do i=1,nmom
c      write(300,'(6e15.6)') (rinv(i,j)*1.e20,j=1,nmom) ! covariance corrected for scaling 1.e20
c      write(300,'(6e15.6)') (rinvprint(i,j),j=1,nmom) ! covariance corrected for scaling 1.e20
c      Multiplication by 1e20 is now in newunc
...
c      write(222,*) 'misfit and sigmas have sense only in newunc'
c      write(222,*) '(when knowing vardat in inpinv), otherwise formal' 
c In isola_NOcova, error3 depend on vardat which is taken from last l;ine of inpinv.dat
c This line is correctly setup only in UNC tool (in newunc folder)
c where vardat value is taken by GUI from last line of inv4.dat 
c In invert tool, vardat is not apriori known so cannot be setup.
c In invert folder, inpinv is denerated by GUI, cannot be manually edited 
c {except the case that the code is ran manually without GUI}       
...
c =================================================================
      ...
      include "elemat_play_NOcova.inc"
      include "oneinv_play_NOcova.inc"
      include "cnsinv_play_NOcova.inc"
