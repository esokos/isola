      program SIPOLNEW4_WISH

c     This is version which should accompany newaspo9_WISH.
c     In that code (waveforms + time shifts) some solutions can violate polarities
c     and, IMPORTANTLY, the solutions with rake and rake-180 have different misfit
c     [because have different tiem shifts].
c     This is a big difference compared to envelopes, where the solution with flipped
c     rake have the same envelopes, hence same misfit.
...
c     IN THIS VERSION WE ACCEPT ONLY SOLUTIONS SATISFYING ALL POLARITIES
c     and we do NOT apply any flip of rake.
C     (IN PREVIOUS VERSIONS WE ACCEPTED ALSO THOSE SOLUTIONS WITH ALL OPPOSITE
c      POLARITY, AND FOR SUCH SOLUTIONS WE FLIPPED THEIR RAKE)
c     !! also in the envelope inversion it was possible that the best-fit
c     !! solution did not agreed with some polarity so that out3 differed from out6
c     !! not only by the flipped rake

c     If the best solution in out6 doe not agree with out3, seismograms
c     need to be recalculated by newaspo_WISH with the s/d/r from out6 !    

c     ....... THE CHANGES ARE ON 2 PLACES, MARKED NEW_JAN2018   	  
...
c     The last column of the output file is a SUM of 'isatis' numbers (ISUM))
...
c!    Using out5.dat, GUI places the best solution in onemech.dat, and others in moremech.dat 
      ...
      open(9,file='out7.dat') ! Output = mean s/d/r over treshold ensemble OR last line of inv2?
	write(4,*) 
	rewind(4)
	write(5,*) 
 	rewind(5)
...
c     output of acceptable solutions WITHOUT polarity check
 ...
 400  format(6(1x,i5),1x,e10.3,5x,i3)
      endif

c     if(abs(isuma).eq.numsta2) then ! old here we accepted either all good OR all bad
      if(    isuma .eq.numsta2) then !!!!!! NEW_JAN2018 : We accept only all good polarities !!!!!!   
...
c     if(abs(isuma).eq.numsta2.and.err.lt.errpos2) then ! DECISION  OLD !!!!
c Here we have solutions which either satisfy OR violate ALL polarities [and err < errpos2]
      if(    isuma .eq.numsta2.and.err.lt.errpos2) then ! DECISION	NEW_JAN2018 ( ALL pol. must be good)  
c Here we have solutions which satisfy all polarities [and err < errpos2]
      if(isuma.lt.0) then ! NEW now this will never happen 
      keyflip=1
...
c output of the BEST  solution WITH polarity check (rake modified if needed)
      ...
      endif ! end of the DECISION
c	  (solutions not satisfying the DECISION criterion do not go to 5 and 55 output files)
...
c Ask Efthimios if he wants the everage separately in out7 (9) or on last line of inv2.dat (55) 
      write(55,
     *   '(1x,i5,1x,f6.2,1x,e12.5,12(1x,f6.0),1x,f6.1,1x,e12.4)')
