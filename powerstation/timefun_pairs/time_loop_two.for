...
c     CHARACTER *3 statname(21)
	...
	dimension seismo(-1000:8192,21,3) !         ??????? is it enough??????????????????????????????????????
...
c      enddo
      ...
      read(151,*) 
      read(151,*) 
      read(151,*) f1,f2,f3,f4	 ! used in  manidata  
      read(151,*)
      read(151,*) vardat
...
c	write(*,*)  str1, dip1, rake1, amoment 
	...
	write(111,*)  '(no meaning for pure volume component)' 
      ...
      write(*,*) 'optimum source_1 shift (in sec)=?'
...
c	 bve(j)=xinv(itim,ir,icom)	* 1.e10		! this is the data column
      ...
      if(keymomcns.eq.1) then
       j=j+1			  !      
       bve(j)=xmomreq12 *1.e-5 	   ! constraining total moment
      endif
...
c###################################
      ...
      if(keymomcns.eq.1) then
      j=j+1
      ama(j,it)=1.  * 1.e-5    ! constraining total moment 
      endif
