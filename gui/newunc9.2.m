...
fprintf(fid,'%s  %s  %s  %s  %s \r\n', 'source', 'misfit', '       var', '    det(Cova)','no_of_rand_samples');
...
for i=1:nsources
      ...
      C_double=C_double./((10^n_exp)^2);
      % output to log file
      fprintf(fid,'%d  %e  %e  %e  %e \r\n', i, misfit, var, det(C_double),no_of_rand_samples(i));
      % clean 
      clear misfit  C_double sum_of_res2  
end
      ...
      %%  get str dip rake etc    
          [m,~]=size(r);
            for ii=1:m
                ...
                hist(adc_1,dcbins);title(['DC% for source ' num2str(i)],'FontSize',18);ax=gca;set(ax,'Linewidth',2,'FontSize',12);
                xlim([0,100]);
                    [n,xout] = hist(adc_1,dcbins);
                ...
                %figure;  %avol_1
                subplot(1,3,2)
                hist(avol_1,xbins);title(['VOL% for source ' num2str(i)],'FontSize',18);ax=gca;set(ax,'Linewidth',2,'FontSize',12);
                xlim([-100,100]);
                   [nv,vxout] = hist(avol_1,xbins);
                    histfilename=['vol_histout_src' num2str(i,'%02u') '.dat'];
                    fid2 = fopen(histfilename,'w');
                        fprintf(fid2,'%f  %f \r\n',[nv; vxout]);
                    fclose(fid2);
        ...
        clear r str1  dp1  rk1  str2  dp2  rk2  adc_1  adc_2  avol_1  avol_2  AA BB n xout nv vxout path 
      else  % deviatoric
...
%                 elipsa_max=make_el_max(AA,BB);
                ...
                subplot(1,2,1)
                %hist(elipsa_max(:,4),10);
                hist(adc_1,dcbins);title(['DC% for source ' num2str(i)],'FontSize',18);ax=gca;
...
%% plot total 
            ...
            if stype==1 % full MT
                ...
                figure; %DC_1
                subplot(1,3,1)
                hist(adc_1,dcbins);title('DC% all sources','FontSize',14);
                ax=gca;set(ax,'Linewidth',2,'FontSize',12);xlim([0,100]);

                subplot(1,3,2)  %avol_1
                hist(avol_1,xbins);title('VOL% all sources ','FontSize',14);
                ...
                toc;title('Nodal plot','FontSize',14)
                %% output
                fid2 = fopen('output_all.dat','w');
                 fprintf(fid2,'%f\r\n',avol_1);
                fclose(fid2);
                [n,xout] = hist(avol_1,xbins);
