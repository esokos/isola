...
% Last Modified by GUIDE v2.5 29-Dec-2018 21:35:03
...
if length(h) == 0; 
  disp('sourcesinfo.txt file doesn''t exist in gmtfiles folder. Using defaults.');
    ...
    if ispc
     ...
     tline = fgetl(fid);
     set(handles.depth,'String',tline)   
     
     tline = fgetl(fid);     
     tline = fgetl(fid);
     set(handles.northkm,'String',tline)
     
     tline = fgetl(fid);     
     tline = fgetl(fid);
     set(handles.eastkm,'String',tline)
     
     tline = fgetl(fid);     
     tline = fgetl(fid);
     set(handles.noSourcesstrike,'String',tline)     
     
     tline = fgetl(fid);     
     tline = fgetl(fid);
     set(handles.distanceStep,'String',tline)      
     
     tline = fgetl(fid);     
     tline = fgetl(fid);
     set(handles.noSourcesdip,'String',tline)        
     
     tline = fgetl(fid);     
     tline = fgetl(fid);
     set(handles.steponPlane,'String',tline)          
     
     tline = fgetl(fid);     
     tline = fgetl(fid);
     set(handles.firstSourcestrike,'String',tline)   
     
     tline = fgetl(fid);    
     tline = fgetl(fid);
     set(handles.firstsourcedip,'String',tline)   
     
     tline = fgetl(fid);    
     tline = fgetl(fid);
     set(handles.Strike,'String',tline)   
     
     tline = fgetl(fid);    
     tline = fgetl(fid);
     set(handles.Dip,'String',tline)   
     ...
     if str2num(tline)==1
         ...
         on =[handles.rake,handles.raketext];
         enableon(on)        
     else
...
case 4             
disp('Calculating sources along inclined line')              

% call subroutine 

[ux,uy,uz,strdis] = calclcoord(strike,dip,rake,noSourcesstrike,distanceStep,firstSourcestrike,epidepth); 
...
% and source.isl !!!!!!!!!
          ...
          cd gmtfiles
             ...
             %%%find map limits...
              border=0.1;
                  wend=min(strdisXgeo); eend=max(strdisXgeo); send=min(strdisYgeo);  nend=max(strdisYgeo);
...
%    
%              figure
%              [min(strdisYgeo)-0.3 max(strdisYgeo)+0.3] 
%              [min(strdisXgeo)-0.3 max(strdisXgeo)+0.3]
%               S = gshhs('gshhs_h.b', [min(strdisYgeo)-0.3 max(strdisYgeo)+0.3], [min(strdisXgeo)-0.3 max(strdisXgeo)+0.3]);
%              
%               lat=S.Lat;
%               lon=S.Lon;
%               
%              
%              [latTrimmed,lonTrimmed] = maptriml(lat, lon,[min(strdisYgeo)-0.3 max(strdisYgeo)+0.3], [min(strdisXgeo)-0.3 max(strdisXgeo)+0.3]);
%              
%              
%              
%             geoshow(latTrimmed,lonTrimmed)
...
off =[handles.rake,handles.raketext];
enableoff(off)
end
