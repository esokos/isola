...
% Last Modified by GUIDE v2.5 23-May-2017 17:33:30
...
disp('This is plotres 14/09/2015');
disp('  ');
...
if length(index_selected)==1   %%% plot ONE or ALL stations ONLY
    ...
    if index_selected == nostations+1                          %%%ALL
          ...
          plotallstations(nostations,staname,normalized,invband,ftime,totime,uselimits,dtime,eventid,addvarred,pbw,net_use,fullstationfile,normsynth)
    elseif index_selected ~= nostations+1                       %%% just one
          disp('Plotting ONLY One Station  Real-Synthetic  data in one Figure')
          stationname=staname{index_selected}
          plotonestation(stationname,normalized,ftime,totime,uselimits,normsynth)
    end
...
function plotallstations(nostations,staname,normalized,invband,ftime,totime,uselimits,dtime,eventid,addvarred,pbw,net_use,fullstationfile,normsynth)
%%%%%prepare filenames  
...
function plotonestation(stationname,normalized,ftime,totime,uselimits,normsynth)
...
if get(handles.full_DC,'Value') == 1  % plot full
    ...
    %%prepare a circle
    % full MT plot based on code by Václav Vavry?uk
    % http://www.ig.cas.cz/en/research-teaching/software-download/
    disp('full mt plot based on codes by Václav Vavrycuk')
...
% write batch
   ...
   % fprintf(fid,'%s\r\n', 'psmeca ref_mech.gmt    -R0/10/0/10 -JX17c -Sa14.6c -K -O  -T0 -W1p >> pol_plot.ps'  );
    ...
    fprintf(fid,'%s\r\n', 'pspolar 4pspolar.txt -R -J -N -Ss0.7c  -D5/5  -M16.1c  -e0.5p  -O -K -T0/0/5/18  >>  pol_plot.ps ');
  ...
  %  fprintf(fid,'%s\r\n', 'psmeca ref_mech.gmt    -R0/10/0/10 -JX17c -Sa14.6c -K -O  -T0 -W1p >> pol_plot.ps'  );
    ...
    fprintf(fid,'%s\r\n', 'gawk "{if (NR>1) print $1,$2,$3,$4}" mypol.dat > 4pspolar.txt');
    fprintf(fid,'%s\r\n', 'pspolar 4pspolar.txt -R -J -N -Ss0.7c  -D5/5  -M14.6c  -e0.5p  -O -K -T0/0/5/18  >>  pol_plot.ps ');
   % fprintf(fid,'%s\r\n', 'pstext ref_mech_text.gmt -R -J -O -K -m -N  >> csps_gmt_plot.ps '  );
    fprintf(fid,'%s\r\n', 'echo .5  2   | psxy -R -J -O  -K -Ss.6c -W0.5p >> pol_plot.ps '  );
    fprintf(fid,'%s\r\n', 'echo .5  1.5 | psxy -R -J -O  -K -Ss.6c  -Gblack  >> pol_plot.ps '  );
    fprintf(fid,'%s\r\n', 'echo .5  1.0 | psxy -R -J -O  -K -Sc.3c  -Gblack  >> pol_plot.ps '  );
